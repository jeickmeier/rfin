...
impl PyMoney {
    ...
    fn __format__(&self, spec: &str) -> PyResult<String> {
        use crate::core::common::fmt::{bare_precision, precision_for, unsupported_spec};

        if spec.is_empty() {
            return Ok(format!("{}", self.inner));
        }
        // Accept ".2f", "2f", or bare ".2"/"2" precision specs.
        let precision = precision_for(spec, 'f', 2)
            .or_else(|| bare_precision(spec, 2))
            .ok_or_else(|| unsupported_spec("Money", spec))?;
        Ok(format!(
            "{} {:.*}",
            self.inner.currency(),
            precision,
