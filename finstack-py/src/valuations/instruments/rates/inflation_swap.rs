...
impl PyInflationSwapBuilder {
    ...
    /// Deprecated: use `inflation_index()` instead.
    #[pyo3(name = "inflation_curve", text_signature = "($self, inflation_curve)")]
    fn inflation_curve_deprecated(
        slf: PyRefMut<'_, Self>,
        inflation_curve: String,
    ) -> PyRefMut<'_, Self> {
        Self::inflation_index(slf, inflation_curve)
    }
    ...
    #[pyo3(text_signature = "($self, base_cpi=None)", signature = (base_cpi=None))]
    fn base_cpi(mut slf: PyRefMut<'_, Self>, base_cpi: Option<f64>) -> PyRefMut<'_, Self> {
        slf.base_cpi = base_cpi;
        slf
    }
    ...
    fn build(slf: PyRefMut<'_, Self>) -> PyResult<PyInflationSwap> {
        ...
        let fixed_rate_val = slf.fixed_rate.unwrap();
...
/// Year-on-year inflation swap binding.
#[pyclass(
    module = "finstack.valuations.instruments",
    name = "YoYInflationSwap",
    frozen,
    from_py_object
)]
#[derive(Clone, Debug)]
pub struct PyYoYInflationSwap {
    pub(crate) inner: Arc<YoYInflationSwap>,
}

impl PyYoYInflationSwap {
    pub(crate) fn new(inner: YoYInflationSwap) -> Self {
        Self {
            inner: Arc::new(inner),
        }
    }
...
impl PyYoYInflationSwapBuilder {
    fn new_with_id(id: InstrumentId) -> Self {
        Self {
            instrument_id: id,
            notional: None,
    ...
    fn ensure_ready(&self) -> PyResult<()> {
        ...
        if self.discount_curve.is_none() {
            return Err(pyo3::exceptions::PyValueError::new_err(
                "discount_curve() is required.",
            ));
        }
        if self.inflation_index_id.as_deref().unwrap_or("").is_empty() {
            return Err(pyo3::exceptions::PyValueError::new_err(
                "inflation_index_id() is required.",
            ));
        }
        Ok(())
    }
...
impl PyYoYInflationSwapBuilder {
    ...
    fn __repr__(&self) -> String {
        "YoYInflationSwapBuilder(...)".to_string()
    }
}
...
impl PyYoYInflationSwap {
    ...
    fn builder<'py>(
        cls: &Bound<'py, PyType>,
        instrument_id: &str,
    ) -> PyResult<Py<PyYoYInflationSwapBuilder>> {
        let py = cls.py();
        let builder = PyYoYInflationSwapBuilder::new_with_id(InstrumentId::new(instrument_id));
        Py::new(py, builder)
    }
    ...
    #[getter]
    fn instrument_type(&self) -> PyInstrumentType {
        PyInstrumentType::new(finstack_valuations::pricer::InstrumentType::YoYInflationSwap)
    }
...
impl fmt::Display for PyYoYInflationSwap {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "YoYInflationSwap({}, fixed_rate={:.4})",
            self.inner.id, self.inner.fixed_rate
        )
    }
}
...
pub(crate) fn register<'py>(
    ...
    module.add_class::<PyYoYInflationSwap>()?;
    module.add_class::<PyYoYInflationSwapBuilder>()?;
    Ok(vec![
        "InflationSwap",
