...
pub(crate) fn portfolio_to_py(err: PortfolioError) -> PyErr {
    match err {
        PortfolioError::UnknownEntity { position_id, entity_id } => {
            PyValueError::new_err(format!("Position '{}' references unknown entity '{}'", position_id, entity_id))
        }
        ...
        PortfolioError::ValuationError { position_id, message } => {
            PyRuntimeError::new_err(format!("Valuation error for position '{}': {}", position_id, message))
        }
        ...
        PortfolioError::Core(err) => {
            PyRuntimeError::new_err(format!("Core error: {}", err))
        }
    }
}
