...
_warnings.warn(
    "finstack.statements.templates is deprecated. "
    "Use finstack.statements_analytics.templates instead.",
    DeprecationWarning,
    stacklevel=2,
)
...
_rust_mod = _finstack.statements.templates

globals().update({k: v for k, v in vars(_rust_mod).items() if not k.startswith("_")})
__all__: list[str] = [n for n in dir(_rust_mod) if not n.startswith("_")]

_sys.modules[__name__] = _sys.modules.get("finstack.statements_analytics.templates", _rust_mod)
