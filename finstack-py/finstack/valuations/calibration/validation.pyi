"""Validation API exposed by :mod:`finstack.valuations.calibration`."""

from __future__ import annotations

from typing import Any

class ValidationConfig:
    def __init__(
        self,
        check_forward_positivity: bool = True,
        min_forward_rate: float = -0.01,
        max_forward_rate: float = 0.50,
        check_monotonicity: bool = True,
        check_arbitrage: bool = True,
        tolerance: float = 1e-10,
        max_hazard_rate: float = 0.50,
        min_cpi_growth: float = -0.10,
        max_cpi_growth: float = 0.50,
        min_fwd_inflation: float = -0.20,
        max_fwd_inflation: float = 0.50,
        max_volatility: float = 5.0,
        allow_negative_rates: bool = False,
        lenient_arbitrage: bool = False,
        butterfly_upper_ratio: float = 1.25,
        butterfly_lower_ratio: float = 0.75,
    ) -> None: ...
    @classmethod
    def standard(cls) -> ValidationConfig: ...
    @classmethod
    def strict(cls) -> ValidationConfig: ...
    @classmethod
    def negative_rates(cls) -> ValidationConfig: ...
    @classmethod
    def lenient(cls) -> ValidationConfig: ...
    def with_lenient_arbitrage(self, lenient: bool) -> ValidationConfig: ...
    @property
    def check_forward_positivity(self) -> bool: ...
    @property
    def min_forward_rate(self) -> float: ...
    @property
    def max_forward_rate(self) -> float: ...
    @property
    def check_monotonicity(self) -> bool: ...
    @property
    def check_arbitrage(self) -> bool: ...
    @property
    def tolerance(self) -> float: ...
    @property
    def max_hazard_rate(self) -> float: ...
    @property
    def min_cpi_growth(self) -> float: ...
    @property
    def max_cpi_growth(self) -> float: ...
    @property
    def min_fwd_inflation(self) -> float: ...
    @property
    def max_fwd_inflation(self) -> float: ...
    @property
    def max_volatility(self) -> float: ...
    @property
    def allow_negative_rates(self) -> bool: ...
    @property
    def lenient_arbitrage(self) -> bool: ...
    @property
    def butterfly_upper_ratio(self) -> float: ...
    @property
    def butterfly_lower_ratio(self) -> float: ...
    def __repr__(self) -> str: ...

def validate_discount_curve(curve: Any, config: ValidationConfig | None = None) -> None: ...
def validate_forward_curve(curve: Any, config: ValidationConfig | None = None) -> None: ...
def validate_hazard_curve(curve: Any, config: ValidationConfig | None = None) -> None: ...
def validate_inflation_curve(curve: Any, config: ValidationConfig | None = None) -> None: ...
def validate_vol_surface(surface: Any, config: ValidationConfig | None = None) -> None: ...
def validate_base_correlation_curve(curve: Any, config: ValidationConfig | None = None) -> None: ...
