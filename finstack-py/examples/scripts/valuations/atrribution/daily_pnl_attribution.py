...
def compare_methods(results):
    ...
    print("-" * 90)
    print(f"{'Total P&L':<30} {str(parallel.total_pnl):<20} {str(waterfall.total_pnl):<20} {str(metrics.total_pnl):<20}")
    print(f"{'Carry':<30} {str(parallel.carry):<20} {str(waterfall.carry):<20} {str(metrics.carry):<20}")
    print(f"{'Rates Curves P&L':<30} {str(parallel.rates_curves_pnl):<20} {str(waterfall.rates_curves_pnl):<20} {str(metrics.rates_curves_pnl):<20}")
    print(f"{'Residual':<30} {str(parallel.residual):<20} {str(waterfall.residual):<20} {str(metrics.residual):<20}")
    print(f"{'Residual %':<30} {parallel.meta.residual_pct:<20.4f} {waterfall.meta.residual_pct:<20.4f} {metrics.meta.residual_pct:<20.4f}")
    print(f"{'Repricings':<30} {parallel.meta.num_repricings:<20} {waterfall.meta.num_repricings:<20} {metrics.meta.num_repricings:<20}")
    
    # Key insights
    print(f"\n{'Key Insights':^90}")
    print("-" * 90)
    print(f"1. Total P&L is consistent across all methods: {parallel.total_pnl}")
    print(f"2. Parallel: {parallel.meta.num_repricings} repricings, {parallel.meta.residual_pct:.2f}% residual (independent factors)")
    print(f"3. Waterfall: {waterfall.meta.num_repricings} repricings, {waterfall.meta.residual_pct:.4f}% residual (sequential, near-zero)")
    print(f"4. Metrics: {metrics.meta.num_repricings} repricings, {metrics.meta.residual_pct:.2f}% residual (fast approximation)")
