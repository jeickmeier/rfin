...
def calculate_irr_from_cashflows(cashflows, initial_investment: float, commitment_date: date = date(2025, 1, 1), debug: bool = False) -> Optional[float]:
...
def analyze_single_scenario(
    ...
    for i, path_result in enumerate(mc_result.path_results):
        ...
        path_irr = calculate_irr_from_cashflows(path_result.cashflows, commitment * initial_utilization, commitment_date)
...
def plot_single_scenario_analysis(results: Dict[str, Any], output_file: Optional[Path] = None):
    ...
    # Plot histogram
    n, bins, patches = ax1.hist(irr_dist, bins=50, alpha=0.7, color='steelblue', 
                                 edgecolor='black', density=True, label='MC Distribution')
    ...
    if results["deterministic_irr"]:
        ...
        ax1.axvline(det_irr_pct, color='red', linestyle='--', linewidth=2, 
                   label=f'Deterministic IRR: {det_irr_pct:.2f}%')
    
    # Add mean and percentiles
    mean_irr = np.mean(irr_dist)
    p5, p50, p95 = np.percentile(irr_dist, [5, 50, 95])
    ax1.axvline(mean_irr, color='green', linestyle=':', linewidth=2, 
               label=f'Mean: {mean_irr:.2f}%')
    ax1.axvline(p50, color='orange', linestyle=':', linewidth=1.5, 
    ...
    if results["utilization_paths"] and results["time_points"]:
        ...
        ax2.fill_between(time_points, p5_path, p95_path, alpha=0.3, color='lightblue', 
    ...
    if results["credit_spread_paths"] and results["time_points"]:
        ...
        ax3.fill_between(time_points, p5_path, p95_path, alpha=0.3, color='moccasin', 
                         label='5th-95th percentile')
...
def export_raw_polars_cashflows(
    ...
    for idx, (path_result, irr) in enumerate(bottom_n, 1):
        ...
        import polars as pl
        df = df.with_columns([
            pl.lit(irr).alias("IRR"),
            pl.lit(f"Bottom_{idx}").alias("Path_Rank")
        ])
    ...
    for idx, (path_result, irr) in enumerate(top_n, 1):
        ...
        df = df.with_columns([
            pl.lit(irr).alias("IRR"),
            pl.lit(f"Top_{idx}").alias("Path_Rank")
        ])
...
def save_cashflow_schedules_to_csv(
                ...
                if hasattr(path_data, 'time_points') and path_data.time_points:
                    ...
                    if 0 <= days_since_start <= 2.0:  # Within facility tenor
                        ...
                        closest_idx = min(range(len(time_points)), 
                                        key=lambda j: abs(time_points[j] - days_since_start))
    ...
    for idx, (path_result, irr) in enumerate(bottom_n, 1):
        ...
        total_interest = sum(f.amount.amount for f in cashflows if 'fixed' in str(f.kind).lower() or 'float' in str(f.kind).lower())
...
def plot_extreme_paths_analysis(
    ...
    fig.suptitle('Cashflow Analysis: Top 5 vs Bottom 5 IRR Paths\n(With Cumulative Cashflows)', 
    ...
    for idx, (path_result, irr) in enumerate(bottom_5):
        ...
        for category in categories:
            ...
            if any(v != 0 for v in pos_values):
                ax_bar.bar(x_positions, pos_values, bottom=bottom_pos,
                          color=colors[category], alpha=0.7, edgecolor='black',
                          width=20)
                bottom_pos += pos_values
                
            if any(v != 0 for v in neg_values):
                ax_bar.bar(x_positions, neg_values, bottom=bottom_neg,
                          color=colors[category], alpha=0.7, edgecolor='black',
                          width=20)
        ...
        for category, cumulative in cumulative_by_category.items():
            ax_cum.plot(x_positions, cumulative,
                       color=colors[category], linewidth=1.5, alpha=0.8,
                       label=category, marker='o', markersize=2)
        ...
        ax_cum.plot(x_positions, total_cumulative,
                   color='black', linewidth=2.5, alpha=0.9,
                   label='Total Net', linestyle='--')
    ...
    stats_text = (f"Bottom 5 Avg: {bottom_avg_irr:.2f}%  |  "
                 f"Top 5 Avg: {top_avg_irr:.2f}%  |  "
                 f"Spread: {spread:.2f}%")
...
def plot_volatility_grid_comparison(
    ...
    scatter = ax3.scatter(util_vols_plot, means, c=cs_vols_plot, s=100, 
