...
def create_balance_sheet_model():
    ...
    ).value("2024Q1", 100_000.0)
    
    builder.node(
        "cash_inflows",
        NodeType.Value(),
        description="Cash inflows from operations"
    ).value("2024Q1", 50_000.0) \
     .value("2024Q2", 55_000.0) \
     .value("2024Q3", 60_000.0)
    
    builder.node(
        "cash_outflows",
        NodeType.Value(),
        description="Cash outflows for operations"
    ).value("2024Q1", 40_000.0) \
     .value("2024Q2", 45_000.0) \
     .value("2024Q3", 50_000.0)
    ...
    builder.node(
     ...
     .value("2024Q2", 30_000.0) \
     .value("2024Q3", 0.0)
    
    builder.node(
        "debt_repayment",
        NodeType.Value(),
        description="Debt principal repayments"
    ).value("2024Q1", 10_000.0) \
     .value("2024Q2", 15_000.0) \
     .value("2024Q3", 20_000.0)
    ...
    builder.node(
        ...
        NodeType.Value(),
        description="EBITDA"
    ).value("2024Q1", 80_000.0) \
     .value("2024Q2", 85_000.0) \
    ...
    ).value("2024Q1", 5_000.0) \
     .value("2024Q2", 5_500.0) \
     .value("2024Q3", 6_000.0)
    
    # Total debt calculation for leverage ratio
    builder.node(
        "total_debt",
        NodeType.Formula("debt"),
        description="Total debt (alias for credit metrics)"
    )
...
def example_1_corkscrew_validation():
    ...
    print(f"Configuration: {repr(corkscrew_config)}")
    ...
    print(f"\nExtension created: {repr(corkscrew)}")
...
def example_2_credit_scorecard():
    ...
    print(f"Configuration: {repr(scorecard_config)}")
    ...
    print(f"\nExtension created: {repr(scorecard)}")
    ...
    print(f"✓ Credit scorecard rating complete")
...
def example_3_configuration_serialization():
    ...
    print(f"\nDeserialized config: {repr(config_restored)}")
    ...
    scorecard_config = ScorecardConfig(
        ...
        metrics=[
            ScorecardMetric("debt_to_ebitda", "debt / ebitda", weight=0.4)
        ],
    )
...
def example_4_runtime_configuration():
    ...
    print(f"Current config: {repr(current_config)}")
    ...
    print(f"Updated config: {repr(updated_config)}")
