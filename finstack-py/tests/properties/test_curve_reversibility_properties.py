...
@st.composite
def discount_curve_strategy(draw):
...
class TestCurveReversibility:
    ...
    def test_parallel_bump_reversible(self, curve, bump_bp, deposit):
    ...
    @settings(max_examples=30, deadline=None)
    def test_symmetric_bumps_cancel(self, curve, bump_bp):
        ...
        for i, (orig, restored) in enumerate(zip(original_dfs, restored_dfs)):
    ...
    def test_multiple_bump_cycles_stable(self, curve, bump_bp, num_cycles):
        """Multiple bump-unbump cycles don't accumulate errors."""
        assume(num_cycles >= 2)
        ...
        for i, (orig, final) in enumerate(zip(original_dfs, final_dfs)):
    ...
    @given(discount_curve_strategy(), small_bumps, deposit_strategy())
    @settings(max_examples=20, deadline=None)
    def test_bump_down_then_up_reversible(self, curve, bump_bp, deposit):
...
class TestCurveBumpCommutativity:
    ...
    @settings(max_examples=20, deadline=None)
    def test_sequential_bumps_additive(self, curve, bump1_bp, bump2_bp):
        ...
        for i, (df_twice, df_combined) in enumerate(zip(dfs_twice, dfs_combined)):
    ...
    @given(discount_curve_strategy(), small_bumps)
    @settings(max_examples=30, deadline=None)
    def test_zero_bump_identity(self, curve, bump_bp):
        ...
        for i, (orig, bump) in enumerate(zip(original_dfs, bumped_dfs)):
            assert abs(orig - bump) < 1e-10, f"Pillar {i}: original {orig}, bumped {bump}"

    @given(discount_curve_strategy(), small_bumps)
    @settings(max_examples=20, deadline=None)
    def test_bump_preserves_curve_structure(self, curve, bump_bp):
...
class TestCurveStability:
    ...
    def test_accumulated_bumps_stable(self, curve, bump_list):
    ...
    @given(discount_curve_strategy(), small_bumps, deposit_strategy())
    @settings(max_examples=20, deadline=None)
    def test_bump_magnitude_affects_pricing(self, curve, bump_bp, deposit):
        ...
        assert abs(pv_bumped - pv_original) > 1e-6 * abs(pv_original), \
            "Bumping curve should affect pricing"
...
class TestCurveInversion:
    ...
    @given(discount_curve_strategy(), small_bumps)
    @settings(max_examples=30, deadline=None)
    def test_bump_inverse_exists(self, curve, bump_bp):
        ...
        for orig, rest in zip(original_dfs, restored_dfs):
            relative_error = abs(rest - orig) / abs(orig) if abs(orig) > 1e-10 else 0
            assert relative_error < 1e-6

    @given(discount_curve_strategy(), small_bumps, small_bumps)
    @settings(max_examples=20, deadline=None)
    def test_bump_order_independent_for_reversal(self, curve, bump1_bp, bump2_bp):
        ...
        for i, (orig, df1, df2) in enumerate(zip(original_dfs, path1_dfs, path2_dfs)):
