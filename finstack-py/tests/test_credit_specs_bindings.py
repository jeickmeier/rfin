...
def test_endogenous_properties() -> None:
    spec = EndogenousHazardSpec.power_law(0.10, 1.5, 2.5)
...
def test_constant_recovery() -> None:
...
def test_optimal_exercise() -> None:
...
def test_toggle_repr() -> None:
    model = ToggleExerciseModel.threshold("leverage", 2.0)
    assert "ToggleExerciseModel" in repr(model)


def test_toggle_invalid_variable() -> None:
    import pytest

    with pytest.raises(ValueError, match=r"invalid_var|variable|Invalid"):
        ToggleExerciseModel.threshold("invalid_var", 0.15)


def test_toggle_invalid_direction() -> None:
    import pytest

    with pytest.raises(ValueError, match=r"direction|sideways|Invalid"):
