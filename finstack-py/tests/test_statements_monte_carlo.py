...
def test_evaluate_monte_carlo_basic() -> None:
    ...
    mc: MonteCarloResults = evaluator.evaluate_monte_carlo(
        model, n_paths=32, seed=7, percentiles=[0.05, 0.5, 0.95]
    )

    assert mc.n_paths == 32
    assert mc.percentiles == [0.05, 0.5, 0.95]
...
def test_evaluate_monte_carlo_deterministic() -> None:
    ...
    assert mc1.percentiles == mc2.percentiles
