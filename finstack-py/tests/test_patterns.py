...
def test_roll_forward_pattern_python() -> None:
    ...
    builder.value_scalar("churn_arr", [
    ...
    assert results.get("arr_beg", PeriodId.quarter(2025, 1)) == 0.0
    assert results.get("arr_end", PeriodId.quarter(2025, 1)) == 90.0

    # Verify Q2: Beg=90, End=90+120-12=198
    assert results.get("arr_beg", PeriodId.quarter(2025, 2)) == 90.0
    assert results.get("arr_end", PeriodId.quarter(2025, 2)) == 198.0

    # Verify Q3: Beg=198, End=198+140-14=324
    assert results.get("arr_beg", PeriodId.quarter(2025, 3)) == 198.0
    assert results.get("arr_end", PeriodId.quarter(2025, 3)) == 324.0
...
def test_vintage_buildup_pattern_python() -> None:
    ...
    builder.value_scalar("new_sales", [
        (PeriodId.quarter(2025, 1), 100.0),
