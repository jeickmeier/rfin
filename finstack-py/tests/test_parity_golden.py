...
def test_period_building():
    ...
    for i, (expected_id, expected_is_actual) in enumerate(zip(expected["period_ids"], expected["is_actual"])):
        assert periods[i].id.code == expected_id
        assert periods[i].is_actual == expected_is_actual


def test_discount_curve_zero_rate():
...
def test_bond_pricing_treasury():
