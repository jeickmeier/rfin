...
def test_weighting_scheme_enum():
...
def test_missing_metric_policy_enum():
...
def test_per_position_metric_construction():
...
def test_metric_expr_construction():
...
def test_objective_construction():
...
def test_position_filter_construction():
...
def test_constraint_metric_bound():
    """Test Constraint.metric_bound (duration <= 4.0)."""
    dur_metric = PerPositionMetric.metric(MetricId.DURATION_MOD)
    dur_expr = MetricExpr.weighted_sum(dur_metric)
    
    constraint = Constraint.metric_bound(
        "duration_limit",
        dur_expr,
        Inequality.LE,
        4.0
...
def test_constraint_tag_exposure_limit():
    """Test Constraint.tag_exposure_limit (CCC exposure <= 10%)."""
    constraint = Constraint.tag_exposure_limit(
        "ccc_limit",
        "rating",
        "CCC",
        0.10
...
def test_constraint_tag_exposure_minimum():
    """Test Constraint.tag_exposure_minimum (IG exposure >= 50%)."""
    constraint = Constraint.tag_exposure_minimum(
        "ig_floor",
        "rating",
        "IG",
        0.50
...
def test_constraint_max_turnover():
...
def test_constraint_max_position_delta():
    """Test Constraint.max_position_delta (max 10% change per position)."""
    filter = PositionFilter.all()
    constraint = Constraint.max_position_delta(
        "delta_limit",
        filter,
        0.10
    )
...
def test_optimize_max_yield_with_ccc_limit_helper():
...
def test_constraint_with_no_label():
...
def test_full_turnover_constraint():
...
def test_api_completeness():
    ...
    print("✓ Portfolio Optimization API is complete")
    print("✓ All constraint types available")
    print("✓ Objective and metric expressions available")
    print("✓ Optimization problem builder available")
    print("✓ Result types and trade generation available")
