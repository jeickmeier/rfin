...
def create_50_pillar_quotes():
    ...
    for tenor, rate in zip(deposit_tenors, deposit_rates):
    ...
    for start, end, rate in zip(fra_starts, fra_ends, fra_rates):
        quotes.append(RatesQuote.fra(start, end, rate / 100.0, "Act360"))
    
    # 3. Futures: 8 contracts (quarterly strip)
    future_dates = [date(2024, 3, 20), date(2024, 6, 19), date(2024, 9, 18),
                    date(2024, 12, 18), date(2025, 3, 19), date(2025, 6, 18),
                    date(2025, 9, 17), date(2025, 12, 17)]
    future_prices = [95.00, 94.90, 94.80, 94.70, 94.60, 94.50, 94.40, 94.30]
    for settle_date, price in zip(future_dates, future_prices):
    ...
    swap_tenors = [
        ...
        "22Y", "23Y", "24Y", "25Y", "27Y", "30Y", "35Y", "40Y", "45Y", "50Y",
    ]
    # Upward-sloping swap curve
    swap_rates = [
        5.25, 5.30, 5.35, 5.40, 5.45, 5.48, 5.50, 5.52, 5.54, 5.56,
        5.58, 5.60, 5.61, 5.62, 5.63, 5.64, 5.65, 5.66, 5.67, 5.68,
        5.69, 5.70, 5.71, 5.72, 5.74, 5.76, 5.80, 5.82, 5.84, 5.85,
    ]
    for tenor, rate in zip(swap_tenors, swap_rates):
        quotes.append(
            RatesQuote.swap(
                tenor,
                rate / 100.0,
...
class TestCurveCalibrationBenchmarks:
    ...
    def test_bench_calibrate_50_pillar_curve(self, benchmark):
    ...
    def test_bench_calibrate_10_pillar_curve(self, benchmark):
        ...
        for tenor, rate in zip(swap_tenors, swap_rates):
            quotes.append(
                RatesQuote.swap(
                    tenor, rate, "SemiAnnual", "Quarterly", "Act360", "Act360"
                )
            )
    ...
    def test_bench_multi_curve_calibration(self, benchmark):
        ...
        for tenor, rate in [("1Y", 0.049), ("2Y", 0.0525), ("5Y", 0.054), ("10Y", 0.0554)]:
            ois_quotes.append(
                RatesQuote.swap(tenor, rate, "SemiAnnual", "Quarterly", "Act360", "Act360")
            )
