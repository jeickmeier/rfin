...
use super::support::{parse_cagr_convention, parse_f64_vec, parse_iso_date, parse_iso_dates};
...
///
/// Returns a JS error for empty returns, non-positive date spans, or invalid
/// annualization factors.
#[wasm_bindgen(js_name = cagr)]
pub fn cagr(returns: JsValue, basis: &WasmCagrBasis) -> Result<f64, JsValue> {
    let r = parse_f64_vec(returns)?;
    fa::risk_metrics::cagr(&r, basis.inner).map_err(to_js_err)
}
