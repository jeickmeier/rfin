...
fn test_model_builder_basic() {
    ...
    // Set periods
    builder = builder.periods("2025Q1..Q4", None).expect("Failed to set periods");
    ...
    builder = builder.value("revenue".to_string(), values.into()).expect("Failed to add value");
    
    // Add a computed node
    builder = builder.compute("cogs".to_string(), "revenue * 0.6".to_string())
...
fn test_evaluator_basic() {
    ...
    builder = builder.periods("2025Q1..Q2", None).expect("Failed to set periods");
    ...
    builder = builder.value("revenue".to_string(), values.into()).expect("Failed to add revenue");
    
    // Add computed nodes
    builder = builder.compute("cogs".to_string(), "revenue * 0.6".to_string())
        .expect("Failed to add cogs");
    builder = builder.compute("gross_profit".to_string(), "revenue - cogs".to_string())
    ...
    // Check results
    let revenue_q1 = results.get("revenue", "2025Q1").expect("Failed to get revenue Q1");
    assert_eq!(revenue_q1, Some(1000000.0));
    
    let gross_profit_q1 = results.get("gross_profit", "2025Q1").expect("Failed to get gross_profit Q1");
...
fn test_registry_builtins() {
    ...
    let metric = registry.get("fin.gross_margin").expect("Failed to get metric");
    let id = metric.id();
...
fn test_model_with_forecast() {
    ...
    builder = builder.periods("2025Q1..Q4", Some("2025Q1".to_string()))
    ...
    builder = builder.value("revenue".to_string(), values.into())
        .expect("Failed to add value");
    
    // Add growth forecast
    let forecast = ForecastSpec::growth(0.05);
    builder = builder.forecast("revenue".to_string(), &forecast)
...
fn test_results_methods() {
    ...
    let values = js_sys::Object::new();
    js_sys::Reflect::set(&values, &wasm_bindgen::JsValue::from_str("2025Q1"), &wasm_bindgen::JsValue::from_f64(100.0)).unwrap();
    js_sys::Reflect::set(&values, &wasm_bindgen::JsValue::from_str("2025Q2"), &wasm_bindgen::JsValue::from_f64(200.0)).unwrap();
    
    builder = builder.value("test_metric".to_string(), values.into()).expect("Failed to add value");
    let model = builder.build().expect("Failed to build model");
    ...
    assert_eq!(results.get_or("nonexistent", "2025Q1", 999.0).unwrap(), 999.0);
...
fn test_json_serialization() {
    ...
    let values = js_sys::Object::new();
    js_sys::Reflect::set(&values, &wasm_bindgen::JsValue::from_str("2025Q1"), &wasm_bindgen::JsValue::from_f64(1000.0)).unwrap();
    
    builder = builder.value("metric".to_string(), values.into()).expect("Failed to add value");
    let model = builder.build().expect("Failed to build model");
