...
use crate::evaluator::{
    context::StatementContext, dag::{DependencyGraph, evaluate_order},
...
impl Evaluator {
    ...
    fn evaluate_period(
        ...
        for node_id in eval_order {
            let node_spec = model.get_node(node_id)
                .ok_or_else(|| Error::NodeNotFound { node_id: node_id.clone() })?;
            ...
            let value = match value_source {
                ...
                NodeValueSource::Forecast => {
                    // Phase 3: Forecast evaluation not yet implemented
                    // For now, return NaN or use formula fallback
                    return Err(Error::eval(format!(
                        "Forecast evaluation not yet implemented for node '{}'",
                        node_id
                    )));
                }
                NodeValueSource::Formula(_formula) => {
                    // Evaluate formula using compiled expression
                    let compiled = self.compiled_cache.get(node_id)
                        .ok_or_else(|| Error::eval(format!("No compiled formula for node '{}'", node_id)))?;
    ...
    fn evaluate_expr(&self, expr: &finstack_core::expr::Expr, context: &StatementContext) -> Result<f64> {
        ...
        match &expr.node {
            ...
            ExprNode::Call(_func, args) => {
                ...
                Err(Error::eval("Function evaluation not fully implemented in Phase 3"))
            }
        }
    }
    ...
    fn evaluate_synthetic_op(
        ...
        let eval_arg = |arg: &finstack_core::expr::Expr| -> Result<f64> {
...
mod tests {
    ...
    fn test_simple_evaluation() {
        ...
        assert_eq!(results.get("revenue", &PeriodId::quarter(2025, 1)), Some(100.0));
        assert_eq!(results.get("revenue", &PeriodId::quarter(2025, 2)), Some(110.0));
    }
    ...
    fn test_formula_evaluation() {
        ...
        // Check gross profit
        assert_eq!(results.get("gross_profit", &PeriodId::quarter(2025, 1)), Some(40.0));
        assert_eq!(results.get("gross_profit", &PeriodId::quarter(2025, 2)), Some(44.0));
    }
