...
fn test_evaluate_value_nodes() {
    ...
    assert_eq!(results.get("revenue", &PeriodId::quarter(2025, 2)), Some(110_000.0));
    assert_eq!(results.get("opex", &PeriodId::quarter(2025, 1)), Some(20_000.0));
    assert_eq!(results.get("opex", &PeriodId::quarter(2025, 2)), Some(21_000.0));
}
...
fn test_evaluate_calculated_nodes() {
    let model = ModelBuilder::new("test")
        ...
        .compute("gross_profit", "revenue - cogs").unwrap()
        .build().unwrap();
    ...
    // Check gross profit
    assert!((results.get("gross_profit", &PeriodId::quarter(2025, 1)).unwrap() - 40_000.0).abs() < 0.01);
    assert!((results.get("gross_profit", &PeriodId::quarter(2025, 2)).unwrap() - 44_000.0).abs() < 0.01);
}

#[test]
fn test_evaluate_arithmetic_operations() {
    let model = ModelBuilder::new("test")
        .periods("2025Q1..Q1", None).unwrap()
        .value("a", &[(PeriodId::quarter(2025, 1), AmountOrScalar::scalar(10.0))])
        .value("b", &[(PeriodId::quarter(2025, 1), AmountOrScalar::scalar(3.0))])
        .compute("add", "a + b").unwrap()
        .compute("sub", "a - b").unwrap()
        .compute("mul", "a * b").unwrap()
        .compute("div", "a / b").unwrap()
        .compute("mod", "a % b").unwrap()
        .build().unwrap();
...
fn test_evaluate_comparison_operations() {
    let model = ModelBuilder::new("test")
        ...
        .compute("gt", "revenue > 50000").unwrap()
        .compute("lt", "revenue < 200000").unwrap()
        .compute("eq", "revenue == 100000").unwrap()
        .build().unwrap();
...
fn test_evaluate_conditional() {
    let model = ModelBuilder::new("test")
        ...
        .compute("bonus", "if(revenue > 100000, revenue * 0.1, 0)").unwrap()
        .build().unwrap();
...
fn test_evaluate_complex_expression() {
    let model = ModelBuilder::new("test")
        .periods("2025Q1..Q1", None).unwrap()
        .value("revenue", &[(PeriodId::quarter(2025, 1), AmountOrScalar::scalar(100_000.0))])
        .value("cogs", &[(PeriodId::quarter(2025, 1), AmountOrScalar::scalar(60_000.0))])
        .compute("gross_margin", "(revenue - cogs) / revenue").unwrap()
        .build().unwrap();
    
    let mut evaluator = Evaluator::new();
    let results = evaluator.evaluate(&model, false).unwrap();
    
    let margin = results.get("gross_margin", &PeriodId::quarter(2025, 1)).unwrap();
...
fn test_dag_simple_chain() {
    let model = ModelBuilder::new("test")
        .periods("2025Q1..Q1", None).unwrap()
        .value("a", &[(PeriodId::quarter(2025, 1), AmountOrScalar::scalar(10.0))])
        .compute("b", "a * 2").unwrap()
        .compute("c", "b + 5").unwrap()
        .build().unwrap();
...
fn test_dag_multiple_dependencies() {
    let model = ModelBuilder::new("test")
        ...
        .value("opex", &[(PeriodId::quarter(2025, 1), AmountOrScalar::scalar(20.0))])
        .compute("cogs", "revenue * 0.6").unwrap()
        .compute("gross_profit", "revenue - cogs").unwrap()
        .compute("operating_income", "gross_profit - opex").unwrap()
        .build().unwrap();
...
fn test_complete_pl_model() {
    let model = ModelBuilder::new("P&L Model")
        .periods("2025Q1..2025Q2", None).unwrap()  // Just Q1-Q2 for Phase 3 (no forecasts yet)
        // Revenue
        .value("revenue", &[
        ...
        .compute("gross_profit", "revenue - cogs").unwrap()
        .compute("operating_income", "gross_profit - opex").unwrap()
        .compute("gross_margin", "gross_profit / revenue").unwrap()
        .build().unwrap();
...
fn test_multiple_periods_sequential() {
    let model = ModelBuilder::new("test")
        ...
        .compute("doubled", "base * 2").unwrap()
        .build().unwrap();
    
    let mut evaluator = Evaluator::new();
    let results = evaluator.evaluate(&model, false).unwrap();
    
    assert_eq!(results.get("doubled", &PeriodId::quarter(2025, 1)), Some(200.0));
    assert_eq!(results.get("doubled", &PeriodId::quarter(2025, 2)), Some(220.0));
    assert_eq!(results.get("doubled", &PeriodId::quarter(2025, 3)), Some(242.0));
...
fn test_nested_parentheses() {
    let model = ModelBuilder::new("test")
        .periods("2025Q1..Q1", None).unwrap()
        .value("revenue", &[(PeriodId::quarter(2025, 1), AmountOrScalar::scalar(100.0))])
        .value("cogs", &[(PeriodId::quarter(2025, 1), AmountOrScalar::scalar(60.0))])
        .value("opex", &[(PeriodId::quarter(2025, 1), AmountOrScalar::scalar(20.0))])
        .compute("result", "((revenue - cogs) - opex) / revenue").unwrap()
        .build().unwrap();
