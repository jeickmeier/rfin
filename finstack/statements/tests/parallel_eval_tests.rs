...
fn test_parallel_evaluation_correctness() {
    ...
    for period in [1, 2, 3, 4] {
        ...
        assert_eq!(
            ...
            "Revenue mismatch for period {:?}", period_id
        );
        ...
        assert_eq!(
            ...
            "COGS mismatch for period {:?}", period_id
        );
        ...
        assert_eq!(
            ...
            "Gross profit mismatch for period {:?}", period_id
        );
        ...
        assert_eq!(
            ...
            "Gross margin mismatch for period {:?}", period_id
        );
    }
}
...
fn test_parallel_disabled_for_time_series() {
    ...
    // Q1: No prior period
    assert!(results.get("qoq_growth", &PeriodId::quarter(2025, 1)).unwrap().is_nan());
    assert!(results.get("lagged_revenue", &PeriodId::quarter(2025, 1)).unwrap().is_nan());
    
    // Q2: Should have correct lag and pct_change
    assert_eq!(
        results.get("qoq_growth", &PeriodId::quarter(2025, 2)).unwrap(),
        0.10  // (110k - 100k) / 100k
    );
    assert_eq!(
        results.get("lagged_revenue", &PeriodId::quarter(2025, 2)).unwrap(),
        100_000.0
    );
    
    // Q3: Should have correct lag and pct_change
    let q3_growth = results.get("qoq_growth", &PeriodId::quarter(2025, 3)).unwrap();
    assert!((q3_growth - 0.0909).abs() < 0.001);  // (120k - 110k) / 110k
    assert_eq!(
        results.get("lagged_revenue", &PeriodId::quarter(2025, 3)).unwrap(),
        110_000.0
    );
}
    ...
    for i in 0..100 {
        ...
        if i == 0 {
            ...
            let values: Vec<_> = (2020..=2030)
                .flat_map(|year| (1..=4).map(move |q| {
                    (PeriodId::quarter(year, q), AmountOrScalar::scalar(1000.0 + i as f64))
                }))
                .collect();
