...
fn test_roll_forward_pattern_arr() -> Result<()> {
    ...
    assert_eq!(results.get("arr_beg", &PeriodId::quarter(2025, 1)), Some(0.0));
    assert_eq!(results.get("arr_end", &PeriodId::quarter(2025, 1)), Some(90.0));

    // Q2: Beg = 90, End = 90 + 120 - 12 = 198
    assert_eq!(results.get("arr_beg", &PeriodId::quarter(2025, 2)), Some(90.0));
    assert_eq!(results.get("arr_end", &PeriodId::quarter(2025, 2)), Some(198.0));

    // Q3: Beg = 198, End = 198 + 140 - 14 = 324
    assert_eq!(results.get("arr_beg", &PeriodId::quarter(2025, 3)), Some(198.0));
    assert_eq!(results.get("arr_end", &PeriodId::quarter(2025, 3)), Some(324.0));
...
fn test_vintage_buildup_pattern_revenue() -> Result<()> {
    ...
    assert_eq!(results.get("revenue", &PeriodId::quarter(2025, 2)), Some(280.0));

    // Q3: New=300. Q2 is 1 old. Q1 is 2 old.
    // Rev = (300*1.0) + (200*0.8) + (100*0.5) = 300 + 160 + 50 = 510
    assert_eq!(results.get("revenue", &PeriodId::quarter(2025, 3)), Some(510.0));

    // Q4: New=400. Q3 is 1 old. Q2 is 2 old. Q1 is 3 old (0.0).
    // Rev = (400*1.0) + (300*0.8) + (200*0.5) + (100*0.0) = 400 + 240 + 100 + 0 = 740
    assert_eq!(results.get("revenue", &PeriodId::quarter(2025, 4)), Some(740.0));
