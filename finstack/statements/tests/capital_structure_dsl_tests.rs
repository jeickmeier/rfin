...
fn test_parse_cs_complex_formula() {
    let ast = parse_formula("(revenue - cogs - opex - cs.interest_expense.total) / revenue").unwrap();
...
fn test_context_get_cs_value_principal_instrument() {
    ...
    cs_cashflows.by_instrument.insert("BOND-001".to_string(), by_instrument);
    
    context.capital_structure_cashflows = Some(cs_cashflows);
    
    // Test getting principal_payment for specific instrument
    let value = context.get_cs_value("principal_payment", "BOND-001").unwrap();
...
fn test_context_get_cs_value_no_cs_error() {
    ...
    let context = StatementContext::new(period_id, node_to_column, historical);
    
    // No capital structure defined - should error
    let result = context.get_cs_value("interest_expense", "total");
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("No capital structure"));
}
...
fn test_context_get_cs_value_invalid_component() {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Unknown capital structure component"));
}
...
fn test_evaluate_model_with_cs_mock() {
    ...
    let model_result = ModelBuilder::new("test")
        ...
        .value("cogs", &[
            ...
            (PeriodId::quarter(2025, 2), AmountOrScalar::scalar(650_000.0)),
        ])
...
fn test_compile_debt_service_ratio() {
    ...
    let expr = parse_and_compile(
        "ebitda / (cs.interest_expense.total + cs.principal_payment.total)"
    ).unwrap();
...
fn test_compile_multiple_instruments() {
    ...
    let expr = parse_and_compile(
        "cs.interest_expense.BOND-001 + cs.interest_expense.SWAP-001"
    ).unwrap();
