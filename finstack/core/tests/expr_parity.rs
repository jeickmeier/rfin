...
fn test_diff_pct_change() {
    ...
    let expected = [f64::NAN, 1.0, 0.5, 1.0/3.0, 0.25, 0.2, 1.0/6.0, 1.0/7.0, 0.125, 1.0/9.0];
...
fn test_cumulative_functions() {
    ...
    let expected = [1.0, 2.0, 6.0, 24.0, 120.0, 720.0, 5040.0, 40320.0, 362880.0, 3628800.0];
...
fn test_rolling_functions() {
    ...
    let rolling_mean_expr = CompiledExpr::new(call(Function::RollingMean, vec![col("values"), lit(3.0)]));
    ...
    let rolling_sum_expr = CompiledExpr::new(call(Function::RollingSum, vec![col("values"), lit(3.0)]));
    let result = rolling_sum_expr.eval_scalar(&ctx, &slices);
    let expected = [f64::NAN, f64::NAN, 6.0, 9.0, 12.0, 15.0, 18.0, 21.0, 24.0, 27.0];
...
fn test_statistical_functions() {
    ...
    for r in &result {
        assert!((r - expected_std).abs() < 1e-4, "std: {} != {}", r, expected_std);
    }
    ...
    for r in &result {
        assert!((r - expected_var).abs() < 1e-4, "var: {} != {}", r, expected_var);
    }
    ...
    for r in &result {
        assert!((r - expected_median).abs() < 1e-10, "median: {} != {}", r, expected_median);
    }
}
...
fn test_ewm_mean() {
    ...
    let ewm_expr = CompiledExpr::new(call(Function::EwmMean, vec![col("values"), lit(0.5), lit(0.0)])); // adjust=false
