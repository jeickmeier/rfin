...
fn quantlib_parity_target2_holidays_2025() {
    ...
    for holiday in expected_holidays {
        assert!(cal.is_holiday(holiday), "TARGET2 2025: {} should be holiday", holiday);
    }
}
...
fn quantlib_parity_nyse_holidays_2024() {
    ...
    for holiday in expected_holidays {
        assert!(cal.is_holiday(holiday), "NYSE: {} should be holiday", holiday);
    }
}
...
fn quantlib_parity_gblo_holidays_2024() {
    ...
    for holiday in expected_holidays {
        assert!(cal.is_holiday(holiday), "GBLO: {} should be holiday", holiday);
    }
}
...
fn quantlib_parity_business_day_following() {
    ...
    assert_eq!(adjusted, expected, "Following: Saturday should roll to Monday");
}
...
fn quantlib_parity_business_day_preceding() {
    ...
    assert_eq!(adjusted, expected, "Preceding: Sunday should roll to Friday");
}
...
fn quantlib_parity_thirty360_february_leap() {
    ...
    assert!((yf - expected_yf).abs() < 0.01, "Expected {}, got {}", expected_yf, yf);
}
...
fn quantlib_parity_act360_month_fractions() {
    ...
    for (start, end, expected) in test_cases {
        let yf = DayCount::Act360
            .year_fraction(start, end, DayCountCtx::default())
            .unwrap();
        assert!((yf - expected).abs() < TOLERANCE, "Failed for {:?} to {:?}", start, end);
    }
}
...
fn quantlib_parity_loglinear_flat_forward_equivalence() {
    ...
    for t in [0.5, 1.5, 2.0, 2.5] {
        ...
        assert!((df_log - df_ff).abs() < TOLERANCE, "LogLinear != FlatFwd at t={}", t);
    }
}
...
fn quantlib_parity_schedule_business_day_adjustment() {
    ...
    assert!(cal.is_business_day(dates[0]), "Adjusted date should be business day");
...
fn quantlib_parity_npv_simple_cashflows() {
    ...
    assert!((npv - expected_npv).abs() < 0.5, "NPV: expected {}, got {}", expected_npv, npv);
}
...
fn quantlib_parity_npv_semiannual_bond() {
    ...
    let curve = DiscountCurve::builder("SEMI")
        .base_date(base)
        .knots(vec![(0.0, 1.0), (0.5, 0.98), (1.0, 0.96), (1.5, 0.94), (2.0, 0.92)])
        .build()
        .unwrap();
...
fn quantlib_parity_lm_solver_parabola() {
    ...
    let objective = |params: &[f64]| {
        (params[0] - 3.0).powi(2) + (params[1] - 4.0).powi(2)
    };
...
fn quantlib_parity_cubic_hermite_monotone_preserving() {
    ...
    for i in 0..50 {
        ...
        let t2 = t1 + 0.1;
        if t2 <= 5.0 {
            assert!(curve.df(t1) >= curve.df(t2), "DF should decrease at t={}", t1);
        }
    }
}
...
fn quantlib_parity_schedule_long_front_stub() {
    ...
    assert!(dates.len() >= 3, "Should have start, combined front, and regular periods");
}
...
fn quantlib_parity_brent_solver_polynomial_roots() {
    ...
    let is_valid_root = (root - 1.0).abs() < 0.01 
        || (root - 2.0).abs() < 0.01 
        || (root - 3.0).abs() < 0.01;
...
fn quantlib_parity_daycount_cross_year_boundary() {
    ...
    let yf_365 = DayCount::Act365F.year_fraction(start, end, DayCountCtx::default()).unwrap();
...
fn quantlib_parity_npv_single_cashflow() {
    ...
    let npv = npv_static(&curve, base, DayCount::Act365F, &flows).unwrap().amount();
...
fn quantlib_parity_npv_quarterly_coupons() {
    ...
    let curve = DiscountCurve::builder("QTR")
        .base_date(base)
        .knots(vec![(0.0, 1.0), (0.25, 0.99), (0.5, 0.98), (0.75, 0.97), (1.0, 0.96)])
        .build()
        .unwrap();
...
fn quantlib_parity_interpolation_forward_rate_positivity() {
    ...
    for style in [InterpStyle::LogLinear, InterpStyle::MonotoneConvex] {
        ...
        for t in [0.5, 1.0, 2.0, 3.0, 5.0] {
            if t > 0.0 {
                let zero = curve.zero(t);
                assert!(zero > 0.0, "{:?} zero rate should be positive at t={}", style, t);
            }
        }
    }
}
