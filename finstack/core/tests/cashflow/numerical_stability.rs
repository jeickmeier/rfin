//! Numerical stability tests for NPV and IRR calculations.
//!
//! These tests verify that calculations maintain precision with:
//! - Large numbers of cashflows
...
/// Tolerance for financial amount comparisons
...
fn npv_100_cashflows_maintains_precision() {
    ...
    let flows: Vec<(Date, Money)> = (0..100)
        .map(|i| {
            let year = 2025 + (i / 12);
            let month = (i % 12) + 1;
            let date = Date::from_calendar_date(year, Month::try_from(month as u8).unwrap(), 1).unwrap();
            (date, Money::new(1000.0, Currency::USD))
        })
        .collect();

    let pv = npv_static(&curve, base, dc, &flows).expect("NPV should succeed");
...
fn npv_500_cashflows_maintains_precision() {
    ...
    assert!(pv.amount() < 50_000.0, "PV should be less than undiscounted sum");
...
fn npv_100_year_cashflow_is_tiny_but_positive() {
    ...
    let flows = vec![(maturity, Money::new(1_000_000.0, Currency::USD))];
    let pv = npv_static(&curve, base, DayCount::Act365F, &flows).expect("NPV should succeed");
    ...
    assert!(pv.amount() > 0.0, "100-year PV must be positive (no underflow)");
    assert!(pv.amount() < expected_pv * 2.0, "100-year PV should be ~{:.0}, got {:.0}", expected_pv, pv.amount());
}
...
// =============================================================================
// Discount Factor Monotonicity Tests
// =============================================================================
...
fn npv_rejects_mixed_currencies() {
    ...
    assert!(
        result.is_err(),
        "NPV should reject mixed currencies"
    );
}
