//! Generated calendar bitset and helper tests
//!
//! Tests for low-level calendar infrastructure:
//! - Bitset operations (bit_test, day_of_year_0_based)
//! - nth_weekday_of_month calculations
//! - Calendar type construction (rule-based, bitset lookup)
...
// ============================================
// Nth Weekday of Month Calculations
// ============================================
...
fn calendar_rule_based_holiday_detection() {
    ...
    let jan1 = make_date(2024, 1, 1); // Monday => observed same day
    assert!(cal.is_holiday(jan1));

    let jan1_2023 = make_date(2023, 1, 1); // Sunday => observed next Monday
...
fn calendar_bitset_lookup_honors_ignore_weekends() {
    let mut bits: YearBits = [0; BITSET_WORDS];
    let mut weekend_day = make_date(BASE_YEAR, 1, 1);
