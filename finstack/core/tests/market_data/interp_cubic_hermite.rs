use finstack_core::market_data::interp::{CubicHermite, InterpFn};
...
fn cubic_hermite_exact_knots() {
    ...
    let interp = CubicHermite::new(knots.clone(), dfs.clone()).unwrap();
...
fn cubic_hermite_monotone_decreasing() {
    let (knots, dfs) = build_inputs();
    let interp = CubicHermite::new(knots.clone(), dfs).unwrap();
