use finstack_core::market_data::interp::{FlatFwd, InterpFn, LogLinearDf, ExtrapolationPolicy};
...
fn logdf_exact_knots() {
    ...
    let interp = LogLinearDf::new(knots.clone(), dfs.clone(), ExtrapolationPolicy::default()).unwrap();
...
fn flat_fwd_matches_logdf() {
    let (knots, dfs) = build_inputs();
    let log_interp = LogLinearDf::new(knots.clone(), dfs.clone(), ExtrapolationPolicy::default()).unwrap();
    let flat = FlatFwd::new(knots, dfs, ExtrapolationPolicy::default()).unwrap();
    for step in 0..=30 {
