...
use finstack_core::expr::{
    Expr, ExprNode, Function, ExecMeta, DagBuilder, PushdownAnalyzer,
};
...
fn test_dag_builder_shared_subexpressions() {
    ...
    let col_node = plan.nodes.iter().find(|n| matches!(n.expr.node, ExprNode::Column(_))).unwrap();
    assert!(col_node.ref_count > 1, "Column should be referenced multiple times");
...
fn test_dag_cost_estimation() {
    ...
    let lag_node = plan.nodes.iter()
        .find(|n| matches!(n.expr.node, ExprNode::Call(Function::Lag, _)))
        .unwrap();
    let rolling_std_node = plan.nodes.iter()
...
fn test_pushdown_boundary_analysis() {
    ...
    assert!(boundaries.estimated_speedup >= 0.0, "Speedup should be non-negative");
...
fn test_dag_cache_strategy() {
    ...
    let expr1 = Expr::call(Function::RollingMean, vec![rolling_std.clone(), Expr::literal(5.0)]);
    let expr2 = Expr::call(Function::RollingSum, vec![rolling_std.clone(), Expr::literal(3.0)]);
    ...
    assert!(rolling_std_node.ref_count > 1, "RollingStd should be shared");
