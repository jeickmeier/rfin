...
fn ast_builders_and_equality_semantics() {
    ...
    if let ExprNode::BinOp { op, .. } = &add.node { assert!(matches!(op, BinOp::Add)); } else { panic!("expected binop"); }
}

#[test]
fn dag_dedup_and_topological_order() {
    let mut builder = finstack_core::expr::dag::DagBuilder::new();
    let expr1 = Expr::call(Function::RollingMean, vec![Expr::column("x"), Expr::literal(3.0)]);
    let expr2 = Expr::call(Function::RollingMean, vec![Expr::column("x"), Expr::literal(3.0)]);
...
fn evaluator_scalar_and_with_plan_cache() {
    ...
    let expr2 = Expr::call(Function::RollingSum, vec![Expr::column("x"), Expr::literal(2.0)]);
    let meta = results_meta(&FinstackConfig::default());
    let compiled2 = CompiledExpr::with_planning(expr2, meta).with_cache(1);
    let out2 = compiled2.eval(&ctx, &cols, EvalOpts { plan: None, cache_budget_mb: Some(1) }).values;
...
fn lag_lead_and_shift_behaviors() {
    ...
    let lag_out = CompiledExpr::new(lag_expr).eval(&ctx, &cols, EvalOpts::default()).values;
    ...
    let lead_out = CompiledExpr::new(lead_expr).eval(&ctx, &cols, EvalOpts::default()).values;
    ...
    let shift_neg = Expr::call(Function::Shift, vec![Expr::column("x"), Expr::literal(-1.0)]);
    let shift_pos_out = CompiledExpr::new(shift_pos).eval(&ctx, &cols, EvalOpts::default()).values;
    let shift_neg_out = CompiledExpr::new(shift_neg).eval(&ctx, &cols, EvalOpts::default()).values;
    for i in 0..4 {
        if i == 0 {
...
fn diff_and_pct_change_with_steps() {
    ...
    let diff2_out = CompiledExpr::new(diff2).eval(&ctx, &cols, EvalOpts::default()).values;
    assert!(diff2_out[0].is_nan() && diff2_out[1].is_nan());
    assert_eq!(diff2_out[2], 3.0);
    assert_eq!(diff2_out[3], 6.0);

    let pct2 = Expr::call(Function::PctChange, vec![Expr::column("x"), Expr::literal(2.0)]);
    let pct2_out = CompiledExpr::new(pct2).eval(&ctx, &cols, EvalOpts::default()).values;
...
fn cumulative_operations() {
    ...
    let s = CompiledExpr::new(cumsum).eval(&ctx, &cols, EvalOpts::default()).values;
    assert_eq!(s, vec![3.0, 4.0, 8.0, 10.0]);

    // cumprod
    let cumprod = Expr::call(Function::CumProd, vec![Expr::column("x")] );
    let p = CompiledExpr::new(cumprod).eval(&ctx, &cols, EvalOpts::default()).values;
    assert_eq!(p, vec![3.0, 3.0, 12.0, 24.0]);

    // cummin / cummax
    let cummin = Expr::call(Function::CumMin, vec![Expr::column("x")] );
    let cmin = CompiledExpr::new(cummin).eval(&ctx, &cols, EvalOpts::default()).values;
    assert_eq!(cmin, vec![3.0, 1.0, 1.0, 1.0]);
    let cummax = Expr::call(Function::CumMax, vec![Expr::column("x")] );
    let cmax = CompiledExpr::new(cummax).eval(&ctx, &cols, EvalOpts::default()).values;
...
fn rolling_statistics_mean_std_var_median_min_max_count() {
    ...
    let rmed = Expr::call(Function::RollingMedian, vec![Expr::column("x"), win.clone()]);
    ...
    let rm = CompiledExpr::new(rmean).eval(&ctx, &cols, EvalOpts::default()).values;
    let rs = CompiledExpr::new(rsum).eval(&ctx, &cols, EvalOpts::default()).values;
    let rst = CompiledExpr::new(rstd).eval(&ctx, &cols, EvalOpts::default()).values;
    let rv = CompiledExpr::new(rvar).eval(&ctx, &cols, EvalOpts::default()).values;
    let rmd = CompiledExpr::new(rmed).eval(&ctx, &cols, EvalOpts::default()).values;
    let rmn = CompiledExpr::new(rmin).eval(&ctx, &cols, EvalOpts::default()).values;
    let rmx = CompiledExpr::new(rmax).eval(&ctx, &cols, EvalOpts::default()).values;
    let rc = CompiledExpr::new(rcount).eval(&ctx, &cols, EvalOpts::default()).values;
...
fn rank_and_quantile() {
    ...
    let r = CompiledExpr::new(rank).eval(&ctx, &cols, EvalOpts::default()).values;
    assert_eq!(r, vec![3.0, 1.0, 2.0, 2.0]);

    let q50 = Expr::call(Function::Quantile, vec![Expr::column("v"), Expr::literal(0.5)]);
    let q = CompiledExpr::new(q50).eval(&ctx, &cols, EvalOpts::default()).values;
...
fn ewm_std_and_var_consistency() {
    ...
    let std_expr = Expr::call(Function::EwmStd, vec![Expr::column("x"), alpha.clone(), adjust_true.clone()]);
    let var_expr = Expr::call(Function::EwmVar, vec![Expr::column("x"), alpha.clone(), adjust_true.clone()]);

    let std_vals = CompiledExpr::new(std_expr).eval(&ctx, &cols, EvalOpts::default()).values;
    let var_vals = CompiledExpr::new(var_expr).eval(&ctx, &cols, EvalOpts::default()).values;
    ...
    for i in 0..std_vals.len() {
        ...
        assert!(diff.abs() < 1e-9, "index {}: std^2={}, var={}", i, std_vals[i] * std_vals[i], var_vals[i]);
