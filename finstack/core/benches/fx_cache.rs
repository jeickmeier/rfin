...
use finstack_core::money::fx::{FxMatrix, FxProvider, FxConversionPolicy, FxRate};
use finstack_core::currency::Currency;
use finstack_core::dates::Date;
use std::collections::HashMap;
use std::sync::Arc;

struct BenchFxProvider { rates: HashMap<(Currency, Currency), f64> }

impl BenchFxProvider { fn new() -> Self {
...
impl FxProvider for BenchFxProvider {
    fn rate(&self, from: Currency, to: Currency, _on: Date, _p: FxConversionPolicy) -> finstack_core::Result<FxRate> {
...
fn bench_fx_cache_hits(c: &mut Criterion) {
    ...
    // Warm cache
    let _ = matrix.rate(Currency::USD, Currency::EUR, d, FxConversionPolicy::CashflowDate).unwrap();
    c.bench_function("fx_cache_hit_usd_eur", |b| {
        b.iter(|| {
            let r = matrix.rate(Currency::USD, Currency::EUR, d, FxConversionPolicy::CashflowDate).unwrap();
            black_box(r)
        })
    });
}
