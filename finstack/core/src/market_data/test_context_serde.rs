...
mod tests {
    ...
    use crate::market_data::term_structures::{
        base_correlation::BaseCorrelationCurve, hazard_curve::HazardCurve,
    };
    use time::Month;
    ...
    fn test_context_with_hazard_curve() {
        ...
        let reconstructed = MarketContext::from_data(data).expect("Should reconstruct from data");

        // Verify the curve exists
        let curve = reconstructed
            .hazard("TEST-HAZARD")
    ...
    fn test_context_with_time_series() {
        ...
        let series = reconstructed
            .series("TEST_SERIES")
            .expect("Should have time series");
        let val = series
            .value_on(Date::from_calendar_date(2025, Month::January, 1).unwrap())
    ...
    fn test_json_round_trip() {
        ...
        let context =
            MarketContext::new().insert_price("TEST_PRICE", MarketScalar::Unitless(100.0));
