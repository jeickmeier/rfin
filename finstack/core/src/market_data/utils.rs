...
// -------------------------------------------------------------------------------------------------
// Deterministic summation helpers
// -------------------------------------------------------------------------------------------------

/// Kahan compensated summation – improves numerical stability while preserving a fixed iteration
/// order. Deterministic for a given input ordering.
#[inline]
pub fn kahan_sum<I>(iter: I) -> F
where
    I: IntoIterator<Item = F>,
{
    let mut sum: F = 0.0;
    let mut c: F = 0.0; // compensation
    for x in iter {
        let y = x - c;
        let t = sum + y;
        c = (t - sum) - y;
...
/// Pairwise (divide-and-conquer) summation over a slice. This uses a balanced reduction tree
/// which yields better accuracy than naive left-to-right accumulation and is deterministic for
/// a fixed input ordering.
pub fn pairwise_sum(xs: &[F]) -> F {
    fn recurse(slice: &[F]) -> F {
        match slice.len() {
            0 => 0.0,
            1 => slice[0],
            2 => slice[0] + slice[1],
