...
pub trait ValidatorExt: Validator + Sized {
    ...
    fn with_condition<F>(self, condition: F, error_msg: &'static str) -> ConditionalValidator<Self, F>
    where
    ...
    B: Validator<Input = A::Output>,
{
    ...
    fn validate(&self, input: &Self::Input) -> ValidationResult<Self::Output> {
        self.first.validate(input).and_then(|intermediate| {
    ...
    fn description(&self) -> Option<&'static str> {
        ...
        self.first.description().or_else(|| self.second.description())
    }
}
...
mod tests {
    ...
    fn conditional_validator() {
        let validator = PositiveValidator.with_condition(|&x| x < 100.0, "Value must be less than 100");
