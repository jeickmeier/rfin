...
impl PeriodId {
    ...
    pub fn quarter(year: i32, q: u8) -> Self { Self { year, index: q, freq: Frequency::Quarterly } }
    /// Build a monthly identifier.
    pub fn month(year: i32, m: u8) -> Self { Self { year, index: m, freq: Frequency::Monthly } }
    /// Build a weekly identifier.
    pub fn week(year: i32, w: u8) -> Self { Self { year, index: w, freq: Frequency::Weekly } }
    /// Build a semi-annual identifier.
    pub fn half(year: i32, h: u8) -> Self { Self { year, index: h, freq: Frequency::SemiAnnual } }
    /// Build an annual identifier.
    pub fn annual(year: i32) -> Self { Self { year, index: 1, freq: Frequency::Annual } }
}
...
pub fn build_periods(range: &str, actuals_until: Option<&str>) -> crate::Result<PeriodPlan> {
    ...
    let periods = ids.drain(..).map(|pid| make_period(pid, actual_cut.as_ref())).collect();
...
fn make_period(pid: PeriodId, cut: Option<&PeriodId>) -> Period {
    ...
    Period { id: pid, start, end, is_actual }
}

fn quarter_bounds(year: i32, q: u8) -> (Date, Date) {
    let (sm, em) = match q { 1 => (Month::January, Month::April), 2 => (Month::April, Month::July), 3 => (Month::July, Month::October), _ => (Month::October, Month::January) };
...
fn month_bounds(year: i32, m: u8) -> (Date, Date) {
    ...
    let start = Date::from_calendar_date(year, sm, 1).unwrap();
    let (ey, em) = if m == 12 { (year + 1, Month::January) } else { (year, Month::try_from(m + 1).unwrap()) };
...
fn half_bounds(year: i32, h: u8) -> (Date, Date) {
    match h {
        1 => (Date::from_calendar_date(year, Month::January, 1).unwrap(),
              Date::from_calendar_date(year, Month::July, 1).unwrap()),
        _ => (Date::from_calendar_date(year, Month::July, 1).unwrap(),
...
fn parse_range(s: &str) -> crate::Result<(PeriodId, PeriodId)> {
    ...
    if parts.len() != 2 { return Err(crate::error::InputError::Invalid.into()); }
    let start = parse_id(parts[0])?;
    let rhs = parts[1].trim();
    // Relative if RHS starts with a letter (Q/M/W/H/A). Absolute if it starts with a digit (YYYY...).
    let end = if rhs.chars().next().map(|c| c.is_ascii_digit()).unwrap_or(false) {
        parse_id(rhs)?
    } else {
        // relative form like "..Q4" / "..M12" / "..W52" / "..H2" / "..A"
        match start.freq {
            Frequency::Quarterly => PeriodId { year: start.year, index: rhs.trim_start_matches('Q').parse().map_err(|_| crate::error::InputError::Invalid)?, freq: Frequency::Quarterly },
            Frequency::Monthly => PeriodId { year: start.year, index: rhs.trim_start_matches('M').parse().map_err(|_| crate::error::InputError::Invalid)?, freq: Frequency::Monthly },
            Frequency::Weekly => PeriodId { year: start.year, index: rhs.trim_start_matches('W').parse().map_err(|_| crate::error::InputError::Invalid)?, freq: Frequency::Weekly },
            Frequency::SemiAnnual => PeriodId { year: start.year, index: rhs.trim_start_matches('H').parse().map_err(|_| crate::error::InputError::Invalid)?, freq: Frequency::SemiAnnual },
            Frequency::Annual => PeriodId { year: start.year, index: 1, freq: Frequency::Annual },
        }
    };
    Ok((start, end))
}
...
fn parse_id(s: &str) -> crate::Result<PeriodId> {
    let s = s.trim();
    if let Some(i) = s.find('Q') { // quarterly
        let year: i32 = s[..i].parse().map_err(|_| crate::error::InputError::Invalid)?;
        let q: u8 = s[i+1..].parse().map_err(|_| crate::error::InputError::Invalid)?;
        if !(1..=4).contains(&q) { return Err(crate::error::InputError::Invalid.into()); }
        return Ok(PeriodId::quarter(year, q));
    }
    if let Some(i) = s.find('M') { // monthly
        let year: i32 = s[..i].parse().map_err(|_| crate::error::InputError::Invalid)?;
        let m: u8 = s[i+1..].parse().map_err(|_| crate::error::InputError::Invalid)?;
        if !(1..=12).contains(&m) { return Err(crate::error::InputError::Invalid.into()); }
        return Ok(PeriodId::month(year, m));
    }
    if let Some(i) = s.find('W') { // weekly
        let year: i32 = s[..i].parse().map_err(|_| crate::error::InputError::Invalid)?;
        let w: u8 = s[i+1..].parse().map_err(|_| crate::error::InputError::Invalid)?;
        if !(1..=53).contains(&w) { return Err(crate::error::InputError::Invalid.into()); }
        return Ok(PeriodId::week(year, w));
    }
    if let Some(i) = s.find('H') { // half-year
        let year: i32 = s[..i].parse().map_err(|_| crate::error::InputError::Invalid)?;
        let h: u8 = s[i+1..].parse().map_err(|_| crate::error::InputError::Invalid)?;
        if !(1..=2).contains(&h) { return Err(crate::error::InputError::Invalid.into()); }
        return Ok(PeriodId::half(year, h));
    }
    if s.chars().all(|c| c.is_ascii_digit()) { // annual
...
fn enumerate_ids(mut cur: PeriodId, end: PeriodId) -> Vec<PeriodId> {
    let mut out = Vec::new();
    while cur <= end { out.push(cur); cur = step(cur); }
    out
}

fn step(mut id: PeriodId) -> PeriodId {
    match id.freq {
        Frequency::Quarterly => { if id.index == 4 { id.year += 1; id.index = 1; } else { id.index += 1; } }
        Frequency::Monthly => { if id.index == 12 { id.year += 1; id.index = 1; } else { id.index += 1; } }
        Frequency::Weekly => { if id.index == 53 { id.year += 1; id.index = 1; } else { id.index += 1; } }
        Frequency::SemiAnnual => { if id.index == 2 { id.year += 1; id.index = 1; } else { id.index += 1; } }
        Frequency::Annual => { id.year += 1; id.index = 1; }
    }
    id
}
...
impl FromStr for PeriodId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> { parse_id(s) }
}
...
mod tests {
    ...
    fn parse_and_enumerate_quarters() {
        ...
        assert_eq!(plan.periods[0].start, Date::from_calendar_date(2025, Month::January, 1).unwrap());
        assert_eq!(plan.periods[2].end, Date::from_calendar_date(2025, Month::October, 1).unwrap());
    }
    ...
    fn parse_and_enumerate_months_across_year() {
        ...
        assert_eq!(plan.periods[0].start, Date::from_calendar_date(2024, Month::November, 1).unwrap());
        assert_eq!(plan.periods[3].end, Date::from_calendar_date(2025, Month::March, 1).unwrap());
    }
    ...
    fn parse_and_enumerate_half_and_annual() {
        ...
        assert_eq!(y.periods[0].start, Date::from_calendar_date(2024, Month::January, 1).unwrap());
        assert_eq!(y.periods[2].end, Date::from_calendar_date(2027, Month::January, 1).unwrap());
    }
}
