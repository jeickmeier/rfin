...
fn test_npv_itm_call_has_higher_value() {
    ...
    assert!(pv.amount() > intrinsic, "ITM call PV should exceed intrinsic");
}
...
fn test_npv_expired_otm_call_is_zero() {
    ...
    assert_approx_eq(pv.amount(), 0.0, 1e-10, 1e-10, "Expired OTM call is worthless");
}
...
fn test_currency_validation_rejects_mismatched_notional() {
    ...
    assert!(result.is_err(), "Should reject mismatched notional currency");
}
...
fn test_price_gk_with_inputs_convenience_method() {
    ...
    let (spot, r_d, r_f, sigma, t) = calc.collect_inputs(&call, &market, as_of).unwrap();
    let pv_explicit = calc.price_gk_with_inputs(&call, spot, r_d, r_f, sigma, t).unwrap();
    
    let pv_standard = calc.npv(&call, &market, as_of).unwrap();

    // Assert: Should match standard npv
    assert_approx_eq(pv_explicit.amount(), pv_standard.amount(), 1e-10, 1e-10, "Explicit pricing matches");
}
