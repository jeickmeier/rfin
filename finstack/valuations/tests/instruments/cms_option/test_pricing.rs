...
fn test_cms_cap_pricing() {
    ...
    assert!(pv.amount() > 0.0, "PV should be positive, got {}", pv.amount());
}
...
fn test_convexity_value() {
    ...
    let result = inst.price_with_metrics(
        &market,
        as_of,
        &[MetricId::ConvexityAdjustmentRisk]
    ).expect("Metric calc failed");
    
    let convexity_val = result.measures.get(MetricId::ConvexityAdjustmentRisk.as_str())
        .copied()
    ...
    assert!(convexity_val >= 0.0, "Convexity adjustment should be non-negative, got {}", convexity_val);
}
