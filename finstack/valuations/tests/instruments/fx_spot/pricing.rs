...
fn test_pricing_consistency_across_dates() {
    ...
    let pv1 = fx.npv(&market, d(2025, 1, 15)).unwrap();
    let pv2 = fx.npv(&market, d(2025, 6, 15)).unwrap();
    let pv3 = fx.npv(&market, d(2026, 1, 15)).unwrap();
...
fn test_multiple_currencies_independence() {
    ...
    let pv_eur = eurusd.npv(&market, test_date()).unwrap();
    let pv_gbp = gbpusd.npv(&market, test_date()).unwrap();
