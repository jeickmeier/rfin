...
use finstack_valuations::instruments::fixed_income::bond::{
    Bond, CallPut, CallPutSchedule, oas_pricer::{OASCalculator, OASPricerConfig, calculate_oas},
};
...
fn test_oas_plain_bond_at_par() {
    ...
    assert!(oas_bp.abs() < 2000.0, "OAS should be reasonable for par bond, got {:.2} bp", oas_bp);
}
...
fn test_oas_plain_bond_discount() {
    ...
    assert!(oas_bp > 0.0, "OAS should be positive for discount bond, got {:.2} bp", oas_bp);
    assert!(oas_bp < 5000.0, "OAS should be finite, got {:.2} bp", oas_bp);
}
...
fn test_oas_convergence_with_tree_steps() {
    ...
    for &step_count in &steps {
        ...
        let oas = calculator.calculate_oas(&bond, &market_context, as_of, 97.5).unwrap();
        oas_values.push(oas);
    }
...
fn test_oas_volatility_sensitivity() {
    ...
    for &vol in &volatilities {
        ...
        let calculator = OASCalculator::with_config(config);
        let oas = calculator.calculate_oas(&bond, &market_context, as_of, 98.0).unwrap();
        oas_values.push(oas);
    }
...
fn test_oas_zero_volatility_limit() {
    ...
    let calculator = OASCalculator::with_config(config);
    let low_vol_oas = calculator.calculate_oas(&bond, &market_context, as_of, 99.0).unwrap();
...
fn test_oas_different_volatilities() {
    ...
    let low_vol_oas = low_vol_calculator.calculate_oas(&bond, &market_context, as_of, 97.0).unwrap();
    let high_vol_oas = high_vol_calculator.calculate_oas(&bond, &market_context, as_of, 97.0).unwrap();
...
fn test_oas_tolerance_convergence() {
    ...
    let tight_oas = tight_calculator.calculate_oas(&bond, &market_context, as_of, 98.0).unwrap();
    let loose_oas = loose_calculator.calculate_oas(&bond, &market_context, as_of, 98.0).unwrap();
