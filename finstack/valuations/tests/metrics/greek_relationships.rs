...
#[test]
fn test_gamma_always_non_negative() {
    // Gamma should always be non-negative for long option positions
    let as_of = date!(2024 - 01 - 01);
    let expiry = date!(2025 - 01 - 01);
...
#[test]
fn test_vega_always_non_negative() {
    // Vega should always be non-negative for long option positions
    let as_of = date!(2024 - 01 - 01);
    let expiry = date!(2025 - 01 - 01);
    let strikes = [80.0, 90.0, 100.0, 110.0, 120.0];
    let spot = 100.0;
