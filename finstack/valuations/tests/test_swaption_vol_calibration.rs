...
use finstack_valuations::calibration::{Calibrator, CalibrationConfig, VolQuote};
...
fn test_swaption_vol_calibration_direct() {
    ...
    assert!(vol_1_5y_3y > 0.0, "Interpolated volatility should be positive");
    assert!(vol_1_5y_3y < 1.0, "Interpolated volatility should be reasonable");
}
...
fn test_swaption_pricing_with_calibrated_surface() {
    ...
    if calibration_result.is_err() {
        eprintln!("Calibration failed: {:?}", calibration_result.as_ref().err());
    }
    ...
    let swaption = Swaption::new_payer(
        ...
        0.04, // 4% strike
        Date::from_calendar_date(2026, Month::January, 1).unwrap(), // 1Y expiry
        Date::from_calendar_date(2026, Month::January, 1).unwrap(), // swap start
        Date::from_calendar_date(2027, Month::January, 1).unwrap(), // swap end (1Y tenor)
        "USD-OIS",
        "USD-OIS",
        "SWAPTION-VOL",
    );
    ...
    assert!(price_result.is_ok(), "Swaption pricing should work with calibrated surface");
    
    let price = price_result.unwrap();
    assert!(price.amount() > 0.0, "Swaption should have positive value");
    assert!(price.amount() < 100_000.0, "Swaption value should be reasonable");
}
...
fn test_insufficient_quotes_error() {
    ...
    assert!(result.is_err(), "Should fail with too few quotes per expiry-tenor");
}
