...
use finstack_core::money::{fx::{FxMatrix, FxProvider, FxConversionPolicy, FxRate}, Money};
...
fn create_test_market_context() -> MarketContext {
    ...
    impl FxProvider for SimpleFxProvider {
        fn rate(&self, from: Currency, to: Currency, _on: Date, _policy: FxConversionPolicy) -> finstack_core::Result<FxRate> {
...
fn test_equity_option_full_integration() {
    ...
    let metrics = registry.compute(&[MetricId::Delta, MetricId::Gamma, MetricId::Vega], &mut context).unwrap();
...
fn test_fx_option_full_integration() {
    ...
    // Test metrics calculation
    let mut registry = MetricRegistry::new();
    finstack_valuations::instruments::options::fx_option::metrics::register_fx_option_metrics(&mut registry);
...
fn test_interest_rate_option_full_integration() {
    ...
    let metrics = registry.compute(&[MetricId::Delta, MetricId::Vega], &mut context).unwrap();
...
fn test_credit_option_full_integration() {
    ...
    let metrics = registry.compute(&[MetricId::Delta, MetricId::Gamma], &mut context).unwrap();
...
fn test_swaption_full_integration() {
    ...
    // Test metrics calculation
    let mut registry = MetricRegistry::new();
    finstack_valuations::instruments::options::swaption::metrics::register_swaption_metrics(&mut registry);
