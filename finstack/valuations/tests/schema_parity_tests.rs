...
fn test_attribution_factors_schema_parity() {
    ...
    let waterfall_variant = method_one_of
        .iter()
        .find(|v| v.get("properties").and_then(|p| p.get("Waterfall")).is_some())
        .expect("Should have Waterfall variant");

    let schema_factors: Vec<&str> = waterfall_variant["properties"]["Waterfall"]["items"]["enum"]
        .as_array()
    ...
    expected.sort();
    let mut actual: Vec<&str> = schema_factors.clone();
    actual.sort();
...
fn test_calibration_step_kinds_schema_parity() {
    ...
    let schema_kinds: Vec<&str> = schema["$defs"]["CalibrationStep"]["properties"]["kind"]["enum"]
        .as_array()
    ...
    expected.sort();
    let mut actual: Vec<&str> = schema_kinds.clone();
    actual.sort();
...
fn test_amortization_spec_schema_parity() {
    ...
    expected.sort();
    let mut actual: Vec<&str> = schema_variants.clone();
    actual.sort();
...
const CANONICAL_IM_METHODOLOGIES: &[&str] =
    &["ClearingHouse", "Haircut", "InternalModel", "Schedule", "Simm"];
...
fn test_margin_im_methodology_schema_parity() {
    ...
    let schema_values: Vec<&str> = schema["definitions"]["ImMethodology"]["enum"]
        .as_array()
    ...
    expected.sort();
    let mut actual: Vec<&str> = schema_values.clone();
    actual.sort();
...
fn test_margin_call_type_schema_parity() {
    ...
    let schema_values: Vec<&str> = schema["definitions"]["MarginCallType"]["enum"]
        .as_array()
...
fn test_margin_tenor_schema_parity() {
    ...
    let schema_values: Vec<&str> = schema["definitions"]["MarginTenor"]["enum"]
        .as_array()
