...
fn cds_quote_id_and_bump_semantics() {
    ...
    match bumped {
        CdsQuote::CdsParSpread { spread_bp, .. } => {
            assert!(
                (spread_bp - 101.0).abs() < F64_ABS_TOL_STRICT,
    ...
    match bumped2 {
        CdsQuote::CdsUpfront {
            ...
            assert!(
                (running_spread_bp - 502.0).abs() < F64_ABS_TOL_STRICT,
                "running spread mismatch: expected 502.0, got {running_spread_bp}"
            );
            assert!(
                (upfront_pct - 0.02).abs() < F64_ABS_TOL_STRICT,
...
fn cds_tranche_quote_id_and_bump_semantics() {
    ...
    match bumped {
        CdsTrancheQuote::CDSTranche {
            ...
            assert!(
                (running_spread_bp - 501.0).abs() < F64_ABS_TOL_STRICT,
                "running spread mismatch: expected 501.0, got {running_spread_bp}"
            );
            assert!(
                (upfront_pct + 2.5).abs() < F64_ABS_TOL_STRICT,
...
fn inflation_quote_maturity_and_bump() {
    ...
    match bumped {
        InflationQuote::InflationSwap { rate, .. } => {
            assert!(
                (rate - 0.0251).abs() < F64_ABS_TOL_STRICT,
    ...
    match bumped2 {
        InflationQuote::YoYInflationSwap {
            ...
            assert!(
                (rate - 0.0295).abs() < F64_ABS_TOL_STRICT,
...
fn vol_quote_bump_and_swaption_maturity_alias() {
    ...
    match bumped {
        VolQuote::OptionVol { vol, .. } => {
            assert!(
                (vol - 0.21).abs() < F64_ABS_TOL_STRICT,
