...
use finstack_valuations::policy::{DSCRSweepPolicy, GridMarginPolicy, IndexFallbackPolicy};
...
fn test_workout_state_transitions() {
    ...
    assert!(engine
        .transition(
            WorkoutState::Stressed {
                indicators: vec!["covenant_breach".to_string()]
            },
            as_of,
    ...
    assert!(engine
        .transition(WorkoutState::Performing, as_of, "Invalid transition",)
        .is_err());
}
...
fn test_recovery_waterfall() {
    ...
    let analysis = engine
        .generate_recovery_flows(outstanding, collateral, as_of)
        .unwrap();
...
fn test_workout_penalty_flows() {
    ...
    assert_eq!(
        penalty_flows[0].kind,
        finstack_valuations::cashflow::primitives::CFKind::Fee
    );
