...
fn test_bond_with_pricing_overrides_and_attributes() {
    ...
    bond.attributes.meta.insert("issuer".to_string(), "ACME Corp".to_string());
    bond.attributes.meta.insert("sector".to_string(), "Technology".to_string());
    ...
    assert_eq!(deserialized.pricing_overrides.quoted_clean_price, Some(98.5));
    ...
    assert_eq!(deserialized.attributes.meta.get("issuer"), Some(&"ACME Corp".to_string()));
    assert_eq!(deserialized.attributes.meta.get("sector"), Some(&"Technology".to_string()));
...
fn test_equity_with_attributes() {
    ...
    equity.attributes.meta.insert("exchange".to_string(), "NASDAQ".to_string());
    ...
    assert_eq!(deserialized.attributes.meta.get("exchange"), Some(&"NASDAQ".to_string()));
...
fn test_equity_option_with_pricing_overrides() {
    ...
    option.pricing_overrides = PricingOverrides::default()
        .with_implied_vol(0.25);
    
    // Serialize and deserialize
    let json = serde_json::to_string(&option).expect("EquityOption should serialize");
    let deserialized: EquityOption = serde_json::from_str(&json).expect("EquityOption should deserialize");
    
    // Verify pricing overrides were preserved
    assert_eq!(deserialized.pricing_overrides.implied_volatility, Some(0.25));
...
fn test_cds_with_upfront_payment() {
    ...
    let json = serde_json::to_string(&pricing_overrides).expect("PricingOverrides should serialize");
    let deserialized: PricingOverrides = serde_json::from_str(&json).expect("PricingOverrides should deserialize");
    
    assert_eq!(deserialized.upfront_payment, Some(Money::new(50_000.0, Currency::USD)));
...
fn test_bond_with_calendar_and_amortization() {
    ...
    bond.attributes.meta.insert("country".to_string(), "Germany".to_string());
    ...
    assert_eq!(deserialized.attributes.meta.get("country"), Some(&"Germany".to_string()));
    assert_eq!(deserialized.pricing_overrides.quoted_clean_price, Some(99.5));
