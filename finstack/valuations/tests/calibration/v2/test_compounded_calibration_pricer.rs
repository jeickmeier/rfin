...
fn test_calibration_pricer_enables_compounding_for_sofr_forward_mode() {
    ...
    assert!(matches!(irs.float.compounding, FloatingLegCompounding::CompoundedInArrears { .. }), "SOFR swap must use compounding for forward calibration");
    assert_eq!(irs.float.forward_curve_id.as_str(), "USD-SOFR-3M", "Forward curve ID must match pricer target");
    assert_eq!(irs.float.discount_curve_id.as_str(), "USD-OIS", "Discount curve ID must match pricer default");
}
...
fn test_compounded_swap_par_npv_invariant() {
    ...
    let mut irs = pricer
        .create_ois_swap(&quote, Money::new(10_000_000.0, Currency::USD), Currency::USD)
        .expect("should build swap");

    // 1. Calculate par rate using our new metric
    use finstack_valuations::metrics::MetricId;
    use finstack_valuations::instruments::common::traits::Instrument;
    let results = irs.price_with_metrics(&ctx, base, &[MetricId::ParRate])
        .expect("should price metrics");
    
    let par_rate = results.measures.get(MetricId::ParRate.as_str())
        .copied()
    ...
    assert!(npv.amount().abs() < 1.0, "Compounded swap NPV at par {} must be < $1", npv.amount());
}
