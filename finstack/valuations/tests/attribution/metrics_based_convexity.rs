...
fn test_bond_convexity_reduces_residual() {
    ...
    assert!(val_t0_second.measures.contains_key(MetricId::Convexity.as_str()));
    assert_eq!(
        *val_t0_second.measures.get(MetricId::Convexity.as_str()).unwrap(),
        8000.0
    );
    
    // Verify first-order doesn't have convexity
    assert!(!val_t0_first.measures.contains_key(MetricId::Convexity.as_str()));
}
...
fn test_valuation_result_supports_all_second_order_metrics() {
    ...
    let meta = finstack_core::config::results_meta(&FinstackConfig::default());
    let val = ValuationResult::stamped_with_meta("TEST", as_of, value, meta)
