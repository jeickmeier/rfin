...
impl SABRModel {
    ...
    pub fn implied_volatility(
        ...
        // First factor
        let factor1 = alpha / (f_mid_beta * (1.0 + (1.0 - beta).powi(2) / 24.0 
            * ((forward / strike).ln()).powi(2) 
        ...
        let factor3 = 1.0 + time_to_expiry * (
    ...
    fn atm_volatility(&self, forward: F, time_to_expiry: F) -> Result<F> {
        ...
        let vol = alpha / f_beta * (1.0 + time_to_expiry * (
    ...
    pub fn implied_volatility_obloj(
        &self,
        forward: F,
        strike: F,
        time_to_expiry: F,
    ...
    pub fn density(
        ...
        let _d2vol_dk2 = (vol_plus - 2.0 * self.implied_volatility(forward, strike, time_to_expiry)? 
            + vol_minus) / bump.powi(2);
...
impl SABRSmile {
    ...
    pub fn generate_smile(&self, strikes: &[F]) -> Result<Vec<F>> {
        ...
        for &strike in strikes {
            let vol = self.model.implied_volatility(self.forward, strike, self.time_to_expiry)?;
            vols.push(vol);
        }
    ...
    pub fn strike_from_delta(&self, delta: F, is_call: bool) -> Result<F> {
        ...
        let atm_vol = self.model.atm_volatility(self.forward, self.time_to_expiry)?;
    ...
    pub fn curvature(&self, strike: F) -> Result<F> {
        ...
        let vol_center = self.model.implied_volatility(
            self.forward,
            strike,
            self.time_to_expiry,
        )?;
...
mod tests {
    ...
    fn test_sabr_atm_volatility() {
        ...
        let implied_vol = model.implied_volatility(forward, forward, time_to_expiry).unwrap();
    ...
    fn test_sabr_smile_shape() {
        ...
        for strike in &strikes {
            let vol = model.implied_volatility(forward, *strike, time_to_expiry).unwrap();
            vols.push(vol);
        }
    ...
    fn test_sabr_calibration() {
        ...
        let params = calibrator.calibrate(
            forward,
            &strikes,
            &market_vols,
            time_to_expiry,
        ...
        let model = SABRModel::new(params);
        for (i, &strike) in strikes.iter().enumerate() {
            let model_vol = model.implied_volatility(forward, strike, time_to_expiry).unwrap();
