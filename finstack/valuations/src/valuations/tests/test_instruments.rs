...
fn fixed_stepup_aligned_and_misaligned_boundaries() {
    ...
    b.principal(init, issue, maturity)
        .fixed_stepup(&[(boundary, 0.05), (maturity, 0.06)], sched, CouponType::Cash);
    let s = b.build().unwrap();

    // Ensure stub detection around boundary
    let has_stub = s.flows.iter().any(|cf| cf.kind == finstack_valuations::cashflow::primitives::CFKind::Stub);
    assert!(has_stub, "Expected a stub period due to misaligned step-up boundary");

    // PV with flat curve DF=1.0 equals sum of flows
    let curve = CoreDiscCurve::builder("USD-OIS").base_date(issue).knots([(0.0, 1.0), (5.0, 1.0)]).linear_df().build().unwrap();
...
fn float_margin_stepup_creates_resets_and_uses_split() {
    ...
    let params = FloatCouponParams { index_id: "SOFR_3M", margin_bp: 0.0, gearing: 1.0, reset_lag_days: 2 };
    let boundary = Date::from_calendar_date(2025, Month::July, 5).unwrap();

    let mut b = cf();
    b.principal(init, issue, maturity)
        .float_margin_stepup(&[(boundary, 450.0), (maturity, 600.0)], params, sched, CouponType::Split { cash_pct: 0.6, pik_pct: 0.4 });
    let s = b.build().unwrap();

    // Ensure float resets are present and PIK flows exist
    let has_resets = s.flows.iter().any(|cf| cf.kind == finstack_valuations::cashflow::primitives::CFKind::FloatReset);
    let has_pik = s.flows.iter().any(|cf| cf.kind == finstack_valuations::cashflow::primitives::CFKind::PIK);
...
fn fixed_to_float_switch_no_double_count_at_switch() {
    ...
    let fixed_sched = ScheduleParams { freq: Frequency::quarterly(), dc: DayCount::Act365F, bdc: BusinessDayConvention::Following, calendar_id: None, stub: StubKind::None };
    let float_sched = ScheduleParams { freq: Frequency::quarterly(), dc: DayCount::Act365F, bdc: BusinessDayConvention::Following, calendar_id: None, stub: StubKind::None };
    let fixed_win = FixedWindow { rate: 0.05, schedule: fixed_sched };
    let float_params = FloatCouponParams { index_id: "SOFR_3M", margin_bp: 600.0, gearing: 1.0, reset_lag_days: 2 };
    let float_win = FloatWindow { params: float_params, schedule: float_sched };
    ...
    assert!(coupons_on_switch.len() <= 2, "At most one coupon kind (and possibly amort/PIK) on switch date");
}
...
fn pik_toggle_window_increases_outstanding_only_in_window() {
    ...
    let sched = ScheduleParams { freq: Frequency::quarterly(), dc: DayCount::Act365F, bdc: BusinessDayConvention::Following, calendar_id: None, stub: StubKind::None };
    ...
    for (d, _m) in path {
        if d >= start_pik && d <= end_pik {
            ...
            if s.flows.iter().any(|cf| cf.date == d && cf.kind == finstack_valuations::cashflow::primitives::CFKind::PIK) { increasing_inside = true; }
        } else if d < start_pik || d > end_pik {
            // Outside window, PIK should not appear
            if s.flows.iter().any(|cf| cf.date == d && cf.kind == finstack_valuations::cashflow::primitives::CFKind::PIK) { non_increasing_before_after = false; }
        }
    }
