...
mod finite_difference_properties {
    ...
    fn test_adaptive_rate_bump_override() {
        ...
        assert!((default_bump - bump_sizes::INTEREST_RATE_BP).abs() < 1e-12,
            "Default should be 1bp: {}", default_bump);
    }
    ...
    fn test_adaptive_bumps_extreme_vol() {
        ...
        assert!((0.001..=0.05).contains(&bump_low), "Low vol bump: {}", bump_low);
        
        // Very high vol
        let bump_high = adaptive_vol_bump(5.0, None);
        assert!((0.001..=0.05).contains(&bump_high), "High vol bump: {}", bump_high);
    }
    ...
    fn test_adaptive_bumps_extreme_time() {
        ...
        assert!((0.001..=0.05).contains(&bump_short), "Short time bump: {}", bump_short);
        
        // Very long time
        let bump_long = adaptive_spot_bump(100.0, 0.3, 100.0, None);
        assert!((0.001..=0.05).contains(&bump_long), "Long time bump: {}", bump_long);
    }
...
#[cfg(test)]
mod edge_cases {
    use crate::metrics::finite_difference::{adaptive_spot_bump, adaptive_vol_bump, central_diff_1d};
    ...
    fn test_bump_order_absolute_value() {
        ...
        assert!((deriv_forward.abs() - deriv_backward.abs()).abs() < 1e-10,
