...
impl SABRCalibrationDerivatives {
    ...
    fn sabr_vol_and_derivatives(&self, strike: F, alpha: F, nu: F, rho: F) -> (F, F, F, F) {
        ...
        let term1 = alpha / (f_mid_power * (1.0 + log_fk * log_fk / 24.0 + log_fk.powi(4) / 1920.0));
        
        let term2_base = 1.0 + t * (
            ((1.0 - beta).powi(2) * alpha * alpha) / (24.0 * f_mid.powf(2.0 * (1.0 - beta)))
    ...
    fn sabr_atm_vol_and_derivatives(&self, alpha: F, nu: F, rho: F) -> (F, F, F, F) {
        ...
        let correction = 1.0 + t * (
            ((1.0 - beta).powi(2) * alpha * alpha) / (24.0 * f.powf(2.0 * (1.0 - beta)))
        ...
        let d_vol_d_alpha = correction / f_power + vol_base * t * (
            ((1.0 - beta).powi(2) * 2.0 * alpha) / (24.0 * f.powf(2.0 * (1.0 - beta)))
            + (rho * beta * nu) / (4.0 * f_power)
        );
        
        let d_vol_d_nu = vol_base * t * (
            (rho * beta * alpha) / (4.0 * f_power)
            + (2.0 - 3.0 * rho * rho) * 2.0 * nu / 24.0
        );
        
        let d_vol_d_rho = vol_base * t * (
            (beta * nu * alpha) / (4.0 * f_power)
            - 6.0 * rho * nu * nu / 24.0
        );
    ...
    fn d_vol_d_alpha_impl(&self, _strike: F, alpha: F, nu: F, rho: F, _vol: F, x: F, term2: F) -> F {
        let f = self.market_data.forward;
        let t = self.market_data.time_to_expiry;
        let beta = self.market_data.beta;
...
mod tests {
    ...
    fn test_gradient_finite_differences() {
        ...
        for i in 0..3 {
            let rel_error = ((analytical_grad[i] - numerical_grad[i]) / numerical_grad[i].max(1e-10)).abs();
