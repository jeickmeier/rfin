...
use crate::calibration::v2::domain::solver::{BootstrapTarget, GlobalSolveTarget};
use crate::calibration::config::CalibrationConfig;
use crate::calibration::v2::domain::pricing::conventions;
...
impl BootstrapTarget for DiscountCurveTarget {
    ...
    fn quote_time(&self, quote: &Self::Quote) -> Result<f64> {
        let dc = conventions::resolve_money_market(&self.pricer, quote.conventions(), self.currency).day_count;
        dc.year_fraction(
            self.base_date,
            quote.maturity_date(),
    ...
    fn initial_guess(&self, quote: &Self::Quote, previous_knots: &[(f64, f64)]) -> Result<f64> {
        ...
        match quote {
            RatesQuote::Deposit { maturity, .. } => {
                ...
                let yf = day_count
                    .year_fraction(
                        if let Some(lag) = quote.conventions().settlement_days {
                            self.base_date + time::Duration::days(lag as i64)
                        } else {
    ...
    fn scan_points(&self, quote: &Self::Quote, initial_guess: f64) -> Result<Vec<f64>> {
        ...
        let mut grid = Self::maturity_aware_scan_grid(
            df_lo,
            df_hi,
            clamped_initial,
            num_points,
    ...
    fn validate_knot(&self, time: f64, value: f64) -> Result<()> {
        if !value.is_finite() || value <= 0.0 {
            return Err(finstack_core::Error::Calibration {
                message: format!("Non-finite or non-positive discount factor at t={:.6}", time),
                category: "bootstrapping".to_string(),
            });
        }
        ...
        if value < lo || value > hi {
            return Err(finstack_core::Error::Calibration {
                message: format!("DF out of bounds at t={:.4}: got {:.6}, range [{:.6}, {:.6}]", time, value, lo, hi),
                category: "bootstrapping".to_string(),
            });
        }
        Ok(())
...
impl GlobalSolveTarget for DiscountCurveTarget {
    ...
    fn build_curve_from_params(
        ...
        let bounds = self.config.effective_rate_bounds(self.currency);
        let mut last_t = 0.0;
        if let Some(spot) = self.spot_knot { last_t = spot.0; }
        ...
        for (&t, &z) in times.iter().zip(params.iter()) {
            ...
            df = df.clamp(self.config.discount_curve.df_hard_min, self.config.discount_curve.df_hard_max);
            knots.push((t, df));
        }
    ...
    fn calculate_residuals(
        ...
        for (i, quote) in quotes.iter().enumerate() {
            if i >= residuals.len() { break; }
            residuals[i] = self.pricer.price_instrument(quote, self.currency, &temp_context)?;
        }
        Ok(())
    }
}
