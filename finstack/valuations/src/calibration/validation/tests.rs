...
fn test_discount_curve_validation() {
    ...
    let valid_curve = finstack_core::market_data::term_structures::discount_curve::DiscountCurve::builder(
        "TEST-VALID",
    )
    .base_date(base_date)
    .knots(vec![
    ...
    let invalid_curve = finstack_core::market_data::term_structures::discount_curve::DiscountCurve::builder(
        "TEST-INVALID",
    )
    .base_date(base_date)
    .knots(vec![
...
fn test_forward_curve_validation() {
    ...
    let valid_curve = ForwardCurve::builder("TEST-FWD", 0.25)
        .base_date(base_date)
        .knots(vec![(0.25, 0.045), (0.5, 0.046), (1.0, 0.047), (2.0, 0.048)])
        .build()
...
fn test_base_correlation_validation() {
    ...
    let valid_curve = BaseCorrelationCurve::builder("TEST-CORR")
        .knots(vec![(3.0, 0.20), (7.0, 0.35), (10.0, 0.45), (15.0, 0.60), (30.0, 0.80)])
        .build()
...
fn test_negative_rate_environment_opt_in() {
    ...
    assert!(permissive_result.is_ok(), "expected ok: {:?}", permissive_result);
}
...
fn test_butterfly_arbitrage_detected_and_fails() {
    ...
    let vol_grid = vec![
        // T=0.25
        0.20, 0.18, 0.20,
        // T=0.5 - extreme butterfly violation
        0.20, 0.50, 0.20,
        // T=1.0
        0.22, 0.20, 0.22,
    ];
...
fn test_calendar_arbitrage_detected_and_fails() {
    ...
    let vol_grid = vec![
        // T=0.25
        0.35, 0.40, 0.35,
        // T=0.5 - lower vol causes calendar arbitrage
        0.18, 0.20, 0.18,
        // T=1.0
        0.20, 0.22, 0.20,
    ];
