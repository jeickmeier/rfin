...
impl MetricCalculator for ImpliedVolCalculator {
    fn calculate(&self, context: &mut MetricContext) -> Result<F> {
        ...
        let market_price = option.pricing_overrides.quoted_clean_price
        ...
        let forward_curve = context.curves.get_forward_ref(option.forward_id.clone())?;
        let discount_curve = context.curves.get_discount_ref(option.disc_id.clone())?;
        
        // For single period caplet/floorlet, use simple calculation
        let time_to_fixing = if option.start_date > context.as_of {
            forward_curve.day_count().year_fraction(context.as_of, option.start_date, finstack_core::dates::DayCountCtx::default())?
        } else {
            0.0
        };
        ...
        let time_to_payment = forward_curve.day_count().year_fraction(context.as_of, option.end_date, finstack_core::dates::DayCountCtx::default())?;
        let discount_factor = discount_curve.df(time_to_payment);
        
        let accrual_fraction = option.day_count.year_fraction(option.start_date, option.end_date, finstack_core::dates::DayCountCtx::default())?;
        let is_cap = matches!(option.rate_option_type, RateOptionType::Cap | RateOptionType::Caplet);
        ...
        if implied_vol > 0.0 && implied_vol < 5.0 {
            ...
            Err(finstack_core::Error::Validation("Unreasonable implied volatility".to_string()))
        }
    }
