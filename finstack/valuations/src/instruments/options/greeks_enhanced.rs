...
impl EnhancedGreeksCalculator {
        ...
        F5: Fn() -> F,
    {
        ...
        let gamma = self.calculate_gamma(|_s| {
...
impl CrossGreeksCalculator {
        ...
        P: Fn(F, F) -> F,
    {
        ...
        if self.base_calculator.use_richardson {
            ...
            let vanna2 = self.calculate_vanna_at_step(&price_fn, spot, vol, h_spot / 2.0, h_vol / 2.0);
        ...
        P: Fn(F) -> F,
    {
        ...
        if self.base_calculator.use_richardson {
            ...
            let volga1 = (price_fn(vol + h1) - 2.0 * price_fn(vol) + price_fn(vol - h1)) / (h1 * h1);
            let volga2 = (price_fn(vol + h2) - 2.0 * price_fn(vol) + price_fn(vol - h2)) / (h2 * h2);
        ...
        P: Fn(F, F) -> F,
    {
        ...
        if t > dt {
            ...
            let gamma_now = (price_fn(spot + h, t) - 2.0 * price_fn(spot, t) + price_fn(spot - h, t)) / (h * h);
            let gamma_later = (price_fn(spot + h, t - dt) - 2.0 * price_fn(spot, t - dt) + price_fn(spot - h, t - dt)) / (h * h);
...
mod tests {
    ...
    fn test_richardson_vs_standard() {
        ...
        println!("Richardson delta: {}, Standard delta: {}", delta_richardson, delta_standard);
        println!("Richardson error: {}, Standard error: {}", (delta_richardson - 4.0).abs(), (delta_standard - 4.0).abs());
        
        // For this simple function, both methods should be very accurate
        assert!((delta_richardson - 4.0).abs() < 1e-4, "Richardson delta {} should be close to 4.0", delta_richardson);
        assert!((delta_standard - 4.0).abs() < 1e-4, "Standard delta {} should be close to 4.0", delta_standard);
    }
    ...
    fn test_gamma_calculation() {
        ...
        println!("Calculated gamma: {}, expected: 2.0, difference: {}", gamma, (gamma - 2.0).abs());
        // Should be very close to 2, but numerical differentiation has inherent errors
        assert!((gamma - 2.0).abs() < 1e-3, "Gamma {} should be close to 2.0", gamma);
    }
    ...
    fn test_speed_calculation() {
        ...
        assert!((speed - 6.0).abs() < 1.0, "Speed {} should be reasonably close to 6.0", speed);
    }
}
