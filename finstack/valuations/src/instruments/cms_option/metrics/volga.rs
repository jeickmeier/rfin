...
impl MetricCalculator for VolgaCalculator {
    fn calculate(&self, context: &mut MetricContext) -> Result<f64> {
        ...
        let curves_vol_up = scale_surface(&context.curves, vol_surface_id.as_str(), 1.0 + vol_bump)?;
        let pv_vol_up = option.npv(&curves_vol_up, as_of)?.amount();

        let curves_vol_down = scale_surface(&context.curves, vol_surface_id.as_str(), 1.0 - vol_bump)?;
