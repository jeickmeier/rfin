...
impl MetricCalculator for VolgaCalculator {
    fn calculate(&self, context: &mut MetricContext) -> Result<f64> {
        ...
        let curves_vol_up = {
            ...
            use std::sync::Arc;
            let bumped_surface = VolSurface::from_grid(vol_id.as_str(), &state.expiries, &state.strikes, &bumped_vols)?;
            curves.surfaces.insert(CurveId::from(vol_id.as_str()), Arc::new(bumped_surface));
            curves
        };
