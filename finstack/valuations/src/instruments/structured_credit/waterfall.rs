...
impl StructuredCreditWaterfall {
    ...
    pub fn distribute(
        ...
        remaining_cash = self.run_interest_waterfall(
            remaining_cash,
            pool,
            tranches,
            &mut allocation,
        ...
        remaining_cash = self.run_excess_spread_waterfall(
            remaining_cash,
            pool,
            tranches,
            &mut allocation,
    ...
    fn run_interest_waterfall(
        ...
        for step in &self.interest_waterfall {
            ...
            available = self.process_waterfall_step(
                step,
                available,
                tranches,
                allocation,
    ...
    fn process_waterfall_step(
        ...
        match step {
            WaterfallStep::TrusteeFees { amount } => {
                ...
                Ok(Money::new(available.amount() - payment, available.currency()))
            },
            
            WaterfallStep::TrancheInterest { tranche_id, include_deferred } => {
                if let Some(tranche) = tranches.tranches.iter().find(|t| t.id.as_str() == tranche_id) {
                    ...
                    return Ok(Money::new(available.amount() - payment, available.currency()));
                }
                Ok(available) // Tranche not found
            },
            
            WaterfallStep::TranchePrincipal { tranche_id, payment_type } => {
                if let Some(tranche) = tranches.tranches.iter().find(|t| t.id.as_str() == tranche_id) {
                    let payment = match payment_type {
                        ...
                        PrincipalPaymentType::ProRata => {
                            ...
                            available.amount() * (tranche.current_balance.amount() / tranches.total_size.amount())
                        },
    ...
    fn determine_payment_mode(&self, coverage_results: &TestResults) -> PaymentMode {
        if !coverage_results.breached_tests.is_empty() {
            ...
            for breach in &coverage_results.breached_tests {
                if breach.consequences_applied.contains(&TriggerConsequence::AccelerateAmortization) {
...
impl WaterfallAllocation {
    ...
    pub fn add_payment(&mut self, recipient: &str, amount: Money, description: &str) {
        ...
        self.payments.insert(recipient.to_string(), PaymentDetail {
            amount,
    ...
    pub fn tranche_payment(&self, tranche_id: &str) -> Option<Money> {
        ...
        let interest = self.payments.get(&interest_key)
            .map(|p| p.amount)
            .unwrap_or_else(|| Money::new(0.0, self.total_available.currency()));
            
        let principal = self.payments.get(&principal_key)
            .map(|p| p.amount)
            .unwrap_or_else(|| Money::new(0.0, self.total_available.currency()));
    ...
    pub fn has_diversions(&self) -> bool {
        self.payments.iter()
            .any(|(_, detail)| detail.description.contains("diverted") || detail.description.contains("turbo"))
    }
}
...
mod tests {
    ...
    fn test_waterfall_allocation_creation() {
        ...
        allocation.add_payment("SENIOR_INTEREST", Money::new(100_000.0, Currency::USD), "Senior interest");
