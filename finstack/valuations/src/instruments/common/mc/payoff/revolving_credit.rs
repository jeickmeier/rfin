...
use super::super::traits::{PathState, Payoff, state_keys};
...
impl Payoff for RevolvingCreditPayoff {
        ...
        if self.defaulted {
            if let Some(default_t) = self.default_time {
                if (current_time - default_t).abs() < 1e-10 {
                    ...
                    self.accumulated_cashflows.push((current_time, recovery_amount - loss));
                }
            }
    ...
    fn value(&self, currency: Currency) -> Money {
        ...
        // Apply sanity bounds
        let bounded_value = value.clamp(-self.commitment_amount * 10.0, self.commitment_amount * 10.0);
