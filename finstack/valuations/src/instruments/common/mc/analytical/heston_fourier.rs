...
use super::super::process::heston::HestonParams;
...
fn heston_characteristic_function(
    ...
    let d_squared_real = sigma_v * sigma_v * (rho * rho * u * u) + (kappa - i_rho_sigma_u).powi(2);
    let d_squared_imag = -sigma_v * sigma_v * u;
    ...
    let ln_mag = (ln_arg_real * ln_arg_real + ln_arg_imag * ln_arg_imag).sqrt().ln();
    let ln_arg = ln_arg_imag.atan2(ln_arg_real);
    
    let one_minus_g_real = 1.0 - g_real;
    let one_minus_g_imag = -g_imag;
    let one_minus_g_mag_sq = one_minus_g_real * one_minus_g_real + one_minus_g_imag * one_minus_g_imag;
    
    let part2_real = -2.0 * (ln_mag * one_minus_g_real) / one_minus_g_mag_sq;
    let part2_imag = -2.0 * (ln_arg * one_minus_g_real - ln_mag * one_minus_g_imag) / one_minus_g_mag_sq;
    
    let c_real = (r - q) * i_u_imag * time + factor * (part1_real + part2_real);
    let c_imag = factor * (part1_imag + part2_imag);

    // D = [(κ - iρσ_v u - d) / σ_v²] * [(1 - exp(-d*T)) / (1 - g*exp(-d*T))]
    let num2_real = (kappa - d_real) * (1.0 - exp_dt_real);
    let num2_imag = (-i_rho_sigma_u - d_imag) * (1.0 - exp_dt_real) + (kappa - d_real) * (-exp_dt_imag);
    
    let d_real_part = (num2_real * term_real + num2_imag * term_imag) / (sigma_v * sigma_v * (term_real * term_real + term_imag * term_imag));
    let d_imag_part = (num2_imag * term_real - num2_real * term_imag) / (sigma_v * sigma_v * (term_real * term_real + term_imag * term_imag));
...
pub fn heston_call_price_fourier(
    ...
    let d1 = ((spot / strike).ln() + (params.r - params.q + 0.5 * variance) * time) / (implied_vol * time.sqrt());
...
mod tests {
    ...
    fn test_heston_put_call_parity() {
        ...
        assert!((lhs - rhs).abs() < 0.01, "Put-call parity failed: {} vs {}", lhs, rhs);
    }
}
