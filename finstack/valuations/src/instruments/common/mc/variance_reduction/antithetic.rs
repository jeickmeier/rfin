...
pub struct AntitheticConfig<'a> {
    pub num_pairs: usize,
    pub time_grid: &'a super::super::time_grid::TimeGrid,
...
fn simulate_path<R, P, D, F>(
    ...
    z: &mut [f64],
    work: &mut [f64],
    time_grid: &super::super::time_grid::TimeGrid,
    shock_sign: f64,
) where
    R: RandomStream,
    P: StochasticProcess,
    ...
    F: Payoff,
{
    ...
    let mut path_state = super::super::traits::PathState::new(0, 0.0);
    if !state.is_empty() {
        path_state.set(super::super::traits::state_keys::SPOT, state[0]);
    }
    ...
    for step in 0..time_grid.num_steps() {
        ...
        if !state.is_empty() {
            path_state.set(super::super::traits::state_keys::SPOT, state[0]);
            if state.len() > 1 {
                path_state.set(super::super::traits::state_keys::VARIANCE, state[1]);
            }
        }
