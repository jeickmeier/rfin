...
impl FirstPassageCalculator {
    ...
    pub fn update(&mut self, credit_spread: f64, dt: f64, current_time: f64) -> DefaultEvent {
...
mod tests {
    ...
    fn test_realistic_low_spread_no_default() {
        ...
        for _ in 0..4 {
            ...
            assert_eq!(
                event,
                DefaultEvent::NoDefault,
                "Should not default with 1bp spread"
            );
        }

        // Cumulative should be tiny
        assert!(
            calc.cumulative_hazard() < 0.001,
            "Cumulative hazard should be very small, got {}",
            calc.cumulative_hazard()
        );
    }
    ...
    fn test_realistic_moderate_spread_low_default() {
        ...
        assert_eq!(
            event,
            DefaultEvent::NoDefault,
            "Should not default with threshold=1.0 > cumulative=0.025"
        );

        assert!(
            (calc.cumulative_hazard() - 0.025).abs() < 1e-5,
            "Cumulative hazard should be 0.025, got {}",
            calc.cumulative_hazard()
        );
    }
}
