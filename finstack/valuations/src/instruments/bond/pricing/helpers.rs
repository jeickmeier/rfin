...
pub fn periods_per_year(
    ...
    match freq {
        finstack_core::dates::Frequency::Months(m) => {
            if m == 0 { return Err(finstack_core::error::InputError::Invalid.into()); }
            Ok(12.0 / (m as finstack_core::F))
        }
        finstack_core::dates::Frequency::Days(d) => {
            if d == 0 { return Err(finstack_core::error::InputError::Invalid.into()); }
...
pub fn df_and_derivative_from_yield(
    ...
    if t <= 0.0 { return Ok((df, 0.0)); }
    let ddf_dy = match comp {
        YieldCompounding::Simple => { let denom = 1.0 + ytm * t; -t / (denom * denom) }
        YieldCompounding::Annual => { -t * df / (1.0 + ytm) }
        YieldCompounding::Periodic(m) => { let m = m as finstack_core::F; -t * df / (1.0 + ytm / m) }
        YieldCompounding::Continuous => { -t * df }
        YieldCompounding::Street => { let m = periods_per_year(bond_freq)?.max(1.0); -t * df / (1.0 + ytm / m) }
    };
    Ok((df, ddf_dy))
}
...
pub fn flows_from_context_or_build(
    ...
    if let Some(flows) = &context.cashflows { return Ok(flows.clone()); }
...
pub fn price_from_ytm_compounded(
    ...
    for &(date, amount) in flows {
        ...
        let t = bond
            ...
            .unwrap_or(0.0);
        if t > 0.0 { let df = df_from_yield(ytm, t, comp, bond.freq)?; pv += amount.amount() * df; }
    }
    Ok(pv)
}
