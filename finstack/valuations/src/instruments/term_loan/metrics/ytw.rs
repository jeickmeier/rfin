...
impl MetricCalculator for YtwCalculator {
    fn calculate(&self, context: &mut MetricContext) -> finstack_core::Result<f64> {
        ...
        if let Some(cs) = &loan.call_schedule {
            ...
            for c in &cs.calls {
                if c.date < as_of || c.date > loan.maturity { continue; }
                let mut outstanding_at = Money::new(0.0, loan.currency);
                for (d, amt) in &out_path { if *d <= c.date { outstanding_at = *amt; } else { break; } }
                let redemption = Money::new(outstanding_at.amount() * (c.price_pct_of_par / 100.0), loan.currency);
        ...
        for (d, amt) in &out_path { if *d <= loan.maturity { final_out = *amt; } }
        ...
        for (exercise_date, redemption) in candidates {
            let y = solve_irr_to_exercise(loan, &schedule, as_of, dirty_now, exercise_date, redemption)?;
            if y < y_worst { y_worst = y; }
        }
        Ok(y_worst)
    }
}
