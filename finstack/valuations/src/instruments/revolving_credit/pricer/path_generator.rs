...
pub fn generate_three_factor_paths(
    ...
    // Convert payment dates to time points
    let time_points = dates_to_times(payment_dates, facility.commitment_date)?;
...
fn build_credit_spread_params(
    ...
    match &mc_config.credit_spread_process {
        CreditSpreadProcessSpec::Cir {
            ...
            const MIN_SPREAD: f64 = 1e-8;
            let stable_initial = initial.max(MIN_SPREAD);
            let stable_theta = theta.max(MIN_SPREAD);

            // Use the parameters as specified by the user
            // The QE discretization will handle Feller violations gracefully
            // by allowing the process to occasionally hit zero
        ...
        CreditSpreadProcessSpec::MarketAnchored {
            ...
            // Use the parameters as calibrated
            // The QE discretization will handle Feller violations gracefully
