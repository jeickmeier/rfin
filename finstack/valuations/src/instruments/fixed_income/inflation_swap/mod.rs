...
mod tests {
    ...
    fn create_test_market_context() -> finstack_core::Result<MarketContext> {
        ...
        let inflation_observations = vec![
            (Date::from_calendar_date(2024, Month::January, 1).unwrap(), 280.0),
            (Date::from_calendar_date(2024, Month::July, 1).unwrap(), 285.0),
            (Date::from_calendar_date(2025, Month::January, 1).unwrap(), 290.0),
        ];
        let inflation_index = InflationIndex::new("US-CPI-U", inflation_observations, Currency::USD)?
    ...
    fn test_zero_pv_at_breakeven_rate() {
        ...
        assert!(pv.amount().abs() < 1.0, "PV should be near zero at breakeven rate, got {}", pv.amount());
    }
    ...
    fn test_ir01_sensitivity() {
        ...
        assert!(ir01.abs() > 10.0, "IR01 should have meaningful magnitude, got {}", ir01);
        
        // For a PayFixed swap, IR01 should typically be negative
        // (higher rates reduce PV of both legs, but affect them differently)
        assert!(ir01 < 0.0, "IR01 should be negative for PayFixed swap, got {}", ir01);
    }
    ...
    fn test_inflation01_sensitivity() {
        ...
        assert!(inflation01 > 0.0, "Inflation01 should be positive for PayFixed swap, got {}", inflation01);
        
        // Should have meaningful magnitude
        assert!(inflation01 > 10.0, "Inflation01 should have meaningful magnitude, got {}", inflation01);
    }
    ...
    fn test_receive_fixed_sensitivities_opposite_signs() {
        ...
        assert!(ir01_receive > 0.0, "IR01 should be positive for ReceiveFixed swap, got {}", ir01_receive);
        assert!(inflation01_receive < 0.0, "Inflation01 should be negative for ReceiveFixed swap, got {}", inflation01_receive);
    }
}
