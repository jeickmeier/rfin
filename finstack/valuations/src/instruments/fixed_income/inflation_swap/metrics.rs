...
impl MetricCalculator for BreakevenCalculator {
    fn calculate(&self, context: &mut MetricContext) -> finstack_core::Result<F> {
        ...
        let inflation_index = context.curves.inflation_index(s.inflation_id).ok_or_else(|| {
...
impl MetricCalculator for Inflation01Calculator {
    fn calculate(&self, context: &mut MetricContext) -> finstack_core::Result<F> {
        ...
        let inflation_sensitivity = s.notional.amount() * (i_maturity_projected / i_start) * df * t_maturity * 0.0001;
