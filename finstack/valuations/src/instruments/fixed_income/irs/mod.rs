...
impl From<InterestRateSwap> for crate::instruments::unified::Instrument {
    fn from(value: InterestRateSwap) -> Self {
        crate::instruments::unified::Instrument::IRS(value)
    }
}
...
impl std::convert::TryFrom<crate::instruments::unified::Instrument> for InterestRateSwap {
    type Error = finstack_core::Error;
    
    fn try_from(value: crate::instruments::unified::Instrument) -> finstack_core::Result<Self> {
        match value {
            crate::instruments::unified::Instrument::IRS(v) => Ok(v),
...
impl IRSBuilder {
    ...
    pub fn build(self) -> finstack_core::Result<InterestRateSwap> {
        ...
        let fixed = FixedLegSpec {
            disc_id: self.fixed_disc_id.ok_or_else(|| finstack_core::Error::from(
                finstack_core::error::InputError::Invalid
            ))?,
            rate: self.fixed_rate.ok_or_else(|| finstack_core::Error::from(
                finstack_core::error::InputError::Invalid
            ))?,
            freq: self.fixed_freq.ok_or_else(|| finstack_core::Error::from(
                finstack_core::error::InputError::Invalid
            ))?,
            dc: self.fixed_dc.ok_or_else(|| finstack_core::Error::from(
                finstack_core::error::InputError::Invalid
            ))?,
            ...
            start: self.fixed_start.ok_or_else(|| finstack_core::Error::from(
                finstack_core::error::InputError::Invalid
            ))?,
            end: self.fixed_end.ok_or_else(|| finstack_core::Error::from(
                finstack_core::error::InputError::Invalid
            ))?,
        };
        ...
        let float = FloatLegSpec {
            disc_id: self.float_disc_id.ok_or_else(|| finstack_core::Error::from(
                finstack_core::error::InputError::Invalid
            ))?,
            fwd_id: self.float_fwd_id.ok_or_else(|| finstack_core::Error::from(
                finstack_core::error::InputError::Invalid
            ))?,
            spread_bp: self.float_spread_bp.unwrap_or(0.0),
            freq: self.float_freq.ok_or_else(|| finstack_core::Error::from(
                finstack_core::error::InputError::Invalid
            ))?,
            dc: self.float_dc.ok_or_else(|| finstack_core::Error::from(
                finstack_core::error::InputError::Invalid
            ))?,
            ...
            start: self.float_start.ok_or_else(|| finstack_core::Error::from(
                finstack_core::error::InputError::Invalid
            ))?,
            end: self.float_end.ok_or_else(|| finstack_core::Error::from(
                finstack_core::error::InputError::Invalid
            ))?,
        };
...
mod tests {
    ...
    fn test_irs_builder_pattern() {
        ...
        let irs = InterestRateSwap::builder()
            ...
            .standard_float_leg("USD-OIS", "USD-SOFR-3M", 0.0, Frequency::quarterly(), DayCount::Act360)
            .build()
            .unwrap();
