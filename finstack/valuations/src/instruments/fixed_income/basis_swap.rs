...
use finstack_core::{
    dates::{Date, DayCount, Frequency, BusinessDayConvention, StubKind, DayCountCtx},
...
impl BasisSwap {
    ...
    fn price_float_leg(
        ...
        for i in 0..schedule.len() - 1 {
            ...
            let t1 = DayCount::Act360.year_fraction(self.start_date, period_start, dc_ctx_for_time)?;
            let t2 = DayCount::Act360.year_fraction(self.start_date, period_end, dc_ctx_for_time)?;
            ...
            let t_end = DayCount::Act360.year_fraction(self.start_date, period_end, dc_ctx_for_df)?;
...
impl Priceable for BasisSwap {
    fn value(&self, context: &MarketContext, valuation_date: Date) -> Result<Money> {
        ...
        Ok(Money::new(primary_pv.amount() - reference_pv.amount(), primary_pv.currency()))
    }
...
mod tests {
    ...
    fn test_basis_swap_pricing() {
        ...
        assert!(pv.amount().abs() < 1000.0, "PV should be small: {}", pv.amount());
    }
}
