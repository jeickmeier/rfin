...
impl YtmSolver {
    ...
    pub fn solve(
        ...
        let price_fn = |y: f64| -> f64 {
            self.calculate_price(cashflows, as_of, y, day_count) - target
        };
        
        let price_derivative = |y: f64| -> f64 {
            self.calculate_price_derivative(cashflows, as_of, y, day_count)
        };
...
pub fn solve_ytm(
    ...
    solver.solve(cashflows, as_of, target_price, day_count, notional, coupon_rate)
}
...
mod tests {
    ...
    fn test_ytm_solver_par_bond() {
        ...
        assert!((ytm - coupon_rate).abs() < 1e-4, "YTM {} should approximately equal coupon rate {}", ytm, coupon_rate);
    }
