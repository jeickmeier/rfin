...
impl IndexFallbackPolicy {
    ...
    pub fn new() -> Self {
        ...
        fallback_chain.insert("USD-LIBOR-3M".to_string(), vec![
            "USD-SOFR-3M".to_string(),
            "USD-OIS".to_string(),
        ]);
        fallback_chain.insert("GBP-LIBOR-3M".to_string(), vec![
            "GBP-SONIA-3M".to_string(),
        ]);
    ...
    pub fn get_rate(
        ...
        self.static_rates.get(index).copied()
...
impl FunctionParam for GridMarginParams {
    ...
    fn to_json(&self) -> finstack_core::Result<String> {
        Ok(format!(
            ...
            self.rating,
            self.sector.as_ref().map(|s| format!(r#""{}""#, s)).unwrap_or("null".to_string()),
            self.maturity_years
        ))
    }
}
    ...
    register_policy(
        ...
        Arc::new(move |params: &dyn Any| {
            if let Some(p) = params.downcast_ref::<GridMarginParams>() {
                let spread = grid_policy.calculate_spread(
                    &p.rating,
                    p.sector.as_deref(),
                    p.maturity_years,
                );
    ...
    register_policy(
        ...
        Arc::new(move |params: &dyn Any| {
            if let Some(p) = params.downcast_ref::<DSCRSweepParams>() {
                let sweep_amount = dscr_policy.calculate_sweep_amount(
                    p.dscr,
                    p.excess_cash,
                    p.minimum_cash,
                );
