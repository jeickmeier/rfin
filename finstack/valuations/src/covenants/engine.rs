...
pub type CustomMetricCalculator = Box<dyn Fn(&MetricContext) -> finstack_core::Result<finstack_core::F> + Send + Sync>;
...
impl CovenantEngine {
    ...
    pub fn register_metric<CalcFn>(&mut self, name: impl Into<String>, calculator: CalcFn) -> &mut Self
    where
        CalcFn: Fn(&MetricContext) -> finstack_core::Result<finstack_core::F> + Send + Sync + 'static,
    {
        self.custom_metrics.insert(name.into(), Box::new(calculator));
        self
    }
        ...
        T: InstrumentMutator,
    {
        ...
        for breach in breaches {
            ...
            let spec = self.specs.iter()
                .find(|s| self.get_covenant_description(&s.covenant.covenant_type) == breach.covenant_type)
            ...
            for consequence in &spec.covenant.consequences {
                ...
                if let Some(historical_breach) = self.breach_history.iter_mut()
                    .find(|b| b.covenant_type == breach.covenant_type && b.breach_date == breach.breach_date)
                {
                    historical_breach.applied_consequences.push(consequence.clone());
                }
            }
        }
    ...
    fn evaluate_spec(
        ...
        let (metric_value, threshold) = match &spec.covenant.covenant_type {
            CovenantType::MaxDebtToEBITDA { threshold } => {
                let debt_to_ebitda = self.get_metric_value(context, &MetricId::custom("debt_to_ebitda"))?;
                (debt_to_ebitda, *threshold)
            },
            CovenantType::MinInterestCoverage { threshold } => {
                let coverage = self.get_metric_value(context, &MetricId::custom("interest_coverage"))?;
                (coverage, *threshold)
            },
            CovenantType::MinFixedChargeCoverage { threshold } => {
                let coverage = self.get_metric_value(context, &MetricId::custom("fixed_charge_coverage"))?;
                (coverage, *threshold)
            },
            CovenantType::MaxTotalLeverage { threshold } => {
                let leverage = self.get_metric_value(context, &MetricId::custom("total_leverage"))?;
                (leverage, *threshold)
            },
            CovenantType::MaxSeniorLeverage { threshold } => {
                let leverage = self.get_metric_value(context, &MetricId::custom("senior_leverage"))?;
                (leverage, *threshold)
            },
            CovenantType::MinAssetCoverage { threshold } => {
                let coverage = self.get_metric_value(context, &MetricId::custom("asset_coverage"))?;
                (coverage, *threshold)
            },
            CovenantType::Custom { metric, test } => {
