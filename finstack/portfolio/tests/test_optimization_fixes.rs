...
use finstack_portfolio::optimization::{
    Objective, PerPositionMetric, PortfolioOptimizationProblem, PortfolioOptimizer,
    WeightingScheme, MetricExpr, DefaultLpOptimizer, CandidatePosition
};
...
fn test_notional_weighting() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let p1 = Position::new(
        "POS_LONG", "ENT_A", "DEP_LONG", Arc::new(dep1), 1.0, PositionUnit::Notional(Some(Currency::USD))
    )?;
    
    let p2 = Position::new(
        "POS_SHORT", "ENT_A", "DEP_SHORT", Arc::new(dep2), -1.0, PositionUnit::Notional(Some(Currency::USD))
    )?;
    ...
    let mut problem = PortfolioOptimizationProblem::new(
        portfolio,
        Objective::Maximize(MetricExpr::WeightedSum { metric: PerPositionMetric::Constant(1.0) }), 
    );
...
fn test_candidate_batching() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut problem = PortfolioOptimizationProblem::new(
        portfolio,
        Objective::Maximize(MetricExpr::WeightedSum { metric: PerPositionMetric::PvBase }),
    );
