...
This skill includes helper scripts in `scripts/` for detecting unused imports across languages. All scripts output JSON and accept one or more file paths as arguments.

- **`find_unused_imports.py`**: Uses AST parsing to detect unused imports in **Python** files.

  ```bash
  python scripts/find_unused_imports.py src/utils.py src/services.py
  ```

- **`find-unused-imports-rust.py`**: Uses regex-based parsing to detect unused `use` statements in **Rust** files. Handles simple, aliased, grouped (`{A, B}`), nested, glob (`*`), and `pub use` re-exports (skipped since they are intentionally unused locally).

  ```bash
  python scripts/find-unused-imports-rust.py src/lib.rs src/models/mod.rs
  ```

- **`find-unused-imports-tsx.py`**: Uses regex-based parsing to detect unused imports in **TypeScript/TSX/JSX/JS** files. Handles default, named, namespace (`* as`), type-only (`import type`), inline type (`{ type Foo }`), and side-effect (`import 'module'`) imports.

  ```bash
  python scripts/find-unused-imports-tsx.py src/App.tsx src/utils/helpers.ts
  ```
...
The skill includes helper scripts for finding unused imports:

```bash
# Find unused imports in Python files
python scripts/find_unused_imports.py src/utils.py src/services.py

# Find unused use statements in Rust files
python scripts/find-unused-imports-rust.py src/lib.rs src/models/mod.rs
