...
```javascript
import { Tenor } from './pkg/rfin_wasm.js';

// Use enum values
const schedule = generateSchedule(
    startDate,
    endDate,
    Tenor.SemiAnnual  // Enum value
);

// In configurations
const bondConfig = {
    tenor: Tenor.Quarterly,
...
async function generatePaymentSchedule(config) {
    ...
    const dates = generateSchedule(startDate, endDate, tenor);
 ...
 * @param {Tenor} [params.tenor=Tenor.SemiAnnual] - Payment tenor
...
class BondBuilder {
    ...
    build() {
        const { notional, currency, couponRate, issueDate, maturityDate, tenor, dayCount } = this.params;
