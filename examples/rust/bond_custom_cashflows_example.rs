...
fn example_stepup_bond() -> finstack_core::Result<()> {
    ...
    let custom_schedule = cf()
        ...
        .fixed_stepup(&[(step1, 0.03), (step2, 0.04), (maturity, 0.05)], schedule_params, CouponType::Cash)
        .build()?;
...
fn example_pik_toggle_bond() -> finstack_core::Result<()> {
    ...
    let custom_schedule = cf()
        ...
        .add_fixed_coupon_window(issue, toggle_date, 0.08, schedule_params, CouponType::Split { cash_pct: 0.5, pik_pct: 0.5 })
        .add_fixed_coupon_window(toggle_date, maturity, 0.07, schedule_params, CouponType::Cash)
        .build()?;
...
fn example_amortizing_bond_with_fees() -> finstack_core::Result<()> {
    ...
    let custom_schedule = cf()
        ...
        .amortization(AmortizationSpec::LinearTo { final_notional: Money::new(10_000_000.0, Currency::EUR) })
    ...
    let disc_curve = DiscountCurve::builder("EUR-OIS")
        .base_date(issue)
        .knots([(0.0, 1.0), (1.0, 0.98), (2.0, 0.95), (3.0, 0.93), (4.0, 0.90), (5.0, 0.87), (6.0, 0.84)])
        .linear_df()
        .build()?;
...
fn example_comparison_regular_vs_custom() -> finstack_core::Result<()> {
    ...
    println!("Custom bond (semi-annual payments): {}", custom_result.value);
