...
use finstack_core::{
    dates::{Date, DayCount, Frequency, BusinessDayConvention},
    market_data::{
...
use finstack_valuations::{
    calibration::{
        bootstrap::DiscountCurveCalibrator,
...
fn demo_multi_curve_mode(base_date: Date) -> Result<()> {
    ...
    println!("  Enforce separation: {}", multi_curve_config.enforce_separation);
...
fn demo_single_curve_mode(base_date: Date) -> Result<()> {
    ...
    println!("Configuration:");
    println!("  Mode: {:?}", multi_curve_config.mode);
    println!("  Single curve tenor: {} years", multi_curve_config.single_curve_tenor);
    println!("  Derive forward from discount: {}", multi_curve_config.derive_forward_from_discount());
