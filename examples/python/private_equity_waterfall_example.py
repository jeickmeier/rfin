...
def main():
    ...
    try:
        ...
        spec = finstack.WaterfallSpec.builder() \
            .style("European") \
            .return_of_capital() \
            .preferred_irr(0.08) \
            .catchup(1.0) \
        ...
        # Compute metrics
        metrics = pe_investment.compute_metrics(["lp_irr", "moic_lp", "dpi_lp", "tvpi_lp"])
        print(f"\nPerformance Metrics:")
    ...
    example_spec = {
        ...
        "tranches": [
            ...
            {"type": "promote_tier", "hurdle": {"irr": {"rate": 0.12}}, "lp_share": 0.8, "gp_share": 0.2},
            {"type": "promote_tier", "hurdle": {"irr": {"rate": 0.15}}, "lp_share": 0.7, "gp_share": 0.3},
        ],
        "irr_basis": "act_365f",
        "catchup_mode": "full"
    }
    ...
    example_events = [
        ...
        {"date": "2022-06-15", "amount": 8_000_000.0, "kind": "proceeds", "deal_id": "Deal_Alpha"},
        {"date": "2024-03-10", "amount": 12_000_000.0, "kind": "proceeds", "deal_id": "Deal_Beta"},
        {"date": "2025-12-31", "amount": 15_000_000.0, "kind": "proceeds", "deal_id": "Deal_Gamma"},
    ]
    ...
    print(f"Total Proceeds: ${sum(e['amount'] for e in example_events if e['kind'] == 'proceeds'):,.0f}")
    total_multiple = sum(e['amount'] for e in example_events if e['kind'] == 'proceeds') / sum(e['amount'] for e in example_events if e['kind'] == 'contribution')
