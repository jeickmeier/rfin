...
def demonstrate_model_valued_bond():
    ...
    print(f"   Issue Date: {bond.issue_date}")
    print(f"   Maturity: {bond.maturity}")
    print(f"   Quoted Price: {'None (Model-valued)' if not bond.quoted_clean_price else bond.quoted_clean_price}")
...
def demonstrate_risk_scenarios():
    ...
    try:
        ...
        convexity_benefit_100bp = 0.5 * convexity_val * (0.01 ** 2) * high_yield.notional.amount
...
def create_bond_portfolio_summary():
    ...
    print(f"{'ID':<14} {'Type':<16} {'Coupon':<8} {'TTM':<6} {'Price':<8} {'Expected':<12}")
    print("-" * 80)
    ...
    for bond_info in bonds_data:
        ...
        print(f"{bond.id:<14} {bond_info['type']:<16} {bond.coupon:>6.1%} "
