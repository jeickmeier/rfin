...
def compare_lag_policies(observations: List[Tuple[Date, float]]):
    ...
    no_lag = InflationIndex(
        "CPI-NO-LAG", observations, Currency("USD"), lag=InflationLag.none()
    )

    # 2-month lag
    lag_2m = InflationIndex(
        "CPI-2M-LAG", observations, Currency("USD"), lag=InflationLag.months(2)
    )

    # 3-month lag
    lag_3m = InflationIndex(
        "CPI-3M-LAG", observations, Currency("USD"), lag=InflationLag.months(3)
    )

    # 90-day lag
    lag_90d = InflationIndex(
        "CPI-90D-LAG", observations, Currency("USD"), lag=InflationLag.days(90)
    )
