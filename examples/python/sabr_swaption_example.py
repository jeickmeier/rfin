...
def sabr_implied_vol(forward, strike, time_to_expiry, alpha, beta, nu, rho):
    ...
    if abs(forward - strike) < 1e-12:
        # ATM case
        f_beta = forward ** beta
        vol = alpha / f_beta * (1 + time_to_expiry * (
    ...
    # First factor
    factor1 = alpha / (f_mid_beta * (1 + (1 - beta)**2 / 24 * np.log(forward / strike)**2
...
def black_swaption_price(forward, strike, annuity, time_to_expiry, volatility, is_payer=True):
    """
...
def calibrate_sabr_beta_fixed(forward, strikes, market_vols, time_to_expiry, beta=0.5):
    ...
    for _ in range(100):
        ...
        model_vols = [sabr_implied_vol(forward, k, time_to_expiry, alpha, beta, nu, rho) 
                      for k in strikes]
...
def main():
    ...
    sabr_vols = [sabr_implied_vol(forward_rate, k, time_to_expiry, alpha, beta, nu, rho) 
                 for k in strikes]
    ...
    for i, strike in enumerate(strikes):
        black_price = black_swaption_price(
            forward_rate, strike, annuity, time_to_expiry, flat_vol
        ) * notional
        
        sabr_price = black_swaption_price(
    ...
    plt.plot(strikes * 100, np.array(flat_vols) * 100, 'r--', label='Flat (Black)', linewidth=2)
    ...
    # Price comparison
    plt.subplot(1, 2, 2)
    black_prices = [black_swaption_price(forward_rate, k, annuity, time_to_expiry, flat_vol) * notional 
                    for k in strikes]
    sabr_prices = [black_swaption_price(forward_rate, k, annuity, time_to_expiry, v) * notional 
                   for k, v in zip(strikes, sabr_vols)]
    
    plt.plot(strikes * 100, np.array(black_prices) / 1000, 'r--', label='Black', linewidth=2)
    plt.plot(strikes * 100, np.array(sabr_prices) / 1000, 'b-', label='SABR', linewidth=2)
    plt.axvline(forward_rate * 100, color='gray', linestyle=':', alpha=0.5, label='ATM')
    plt.xlabel('Strike (%)')
    ...
    market_vols = [sabr_implied_vol(forward_rate, k, time_to_expiry, alpha, beta, nu, rho) 
    ...
    cal_vols = [sabr_implied_vol(forward_rate, k, time_to_expiry, cal_alpha, beta, cal_nu, cal_rho) 
                for k in market_strikes]
    ...
    bump = 0.0001
    vol_up = sabr_implied_vol(forward_rate, forward_rate + bump, time_to_expiry, alpha, beta, nu, rho)
    vol_down = sabr_implied_vol(forward_rate, forward_rate - bump, time_to_expiry, alpha, beta, nu, rho)
    skew = (vol_up - vol_down) / (2 * bump)
    
    # Curvature (second derivative)
    vol_center = sabr_implied_vol(forward_rate, forward_rate, time_to_expiry, alpha, beta, nu, rho)
    ...
    vol_25d_put = sabr_implied_vol(forward_rate, k_25d_put, time_to_expiry, alpha, beta, nu, rho)
    vol_25d_call = sabr_implied_vol(forward_rate, k_25d_call, time_to_expiry, alpha, beta, nu, rho)
