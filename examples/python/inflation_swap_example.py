...
def main():
    ...
    discount_curve = (
        fs.DiscountCurve.builder("USD-OIS")
        .base_date(as_of)
    ...
    inflation_index = fs.InflationIndex.new(
        "US-CPI-U", inflation_observations, fs.Currency.USD
    ).with_lag(
        fs.InflationLag.Months(3)
    )  # Standard 3-month lag

    # Create forward inflation curve (market-implied inflation expectations)
    # Points: (time_years, projected_cpi_level)
    inflation_curve = (
        fs.InflationCurve.builder("US-CPI-U")
        .base_cpi(290.0)
    ...
    print(
        f"✓ Created swaps with {notional.amount()/1e6:.1f}M {notional.currency()} notional"
    )
    ...
    print(
        f"Difference:            ${(pv_pay_fixed.amount() - pv_receive_fixed.amount()):>12,.0f}"
    )
