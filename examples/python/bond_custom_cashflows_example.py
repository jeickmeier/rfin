...
def example_stepup_bond():
    ...
    builder.principal(fs.Money(1_000_000, fs.Currency("USD")), issue, maturity)
...
def example_pik_toggle_bond():
    ...
    builder = CashflowBuilder()
    builder.principal(fs.Money(10_000_000, fs.Currency("USD")), issue, maturity)
...
def example_amortizing_bond():
    ...
    builder = CashflowBuilder()
    builder.principal(fs.Money(50_000_000, fs.Currency("EUR")), issue, maturity)

    # Linear amortization to 20% of original
    builder.with_amortization(
        Amortization.linear_to(fs.Money(10_000_000, fs.Currency("EUR")))
    )
...
def example_dataframe_analysis():
    ...
    builder = CashflowBuilder()
    builder.principal(fs.Money(5_000_000, fs.Currency("USD")), issue, maturity)
    builder.fixed_coupon(
        rate=0.06,
        frequency=fs.Frequency.Quarterly,
        day_count=fs.DayCount.act360(),
