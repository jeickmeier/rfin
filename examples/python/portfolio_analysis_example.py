...
from finstack.instruments import (
    Bond, InterestRateSwap, PayReceive, FixedLeg, FloatLeg
)
...
def create_bond_portfolio():
    ...
    builder.with_amortization(
        Amortization.linear_to_zero(Currency("USD"))
    )
...
def create_risk_report():
    ...
    print("  Max bucket: ${:,.0f} [Limit: $10,000] ✓".format(df['Net_DV01'].abs().max()))
    print("  2Y-5Y concentration: ${:,.0f} [Limit: $20,000] ✓".format(
