...
impl PyCurrency {
    ...
    fn new(code: &str) -> PyResult<Self> {
        let currency = code.parse::<CoreCurrency>()
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(format!("Invalid currency code: {}", e)))?;
    ...
    fn usd(_cls: &Bound<'_, PyType>) -> Self {
        PyCurrency { inner: CoreCurrency::USD }
    }

    #[classmethod]
    fn eur(_cls: &Bound<'_, PyType>) -> Self {
        PyCurrency { inner: CoreCurrency::EUR }
    }

    #[classmethod]
    fn gbp(_cls: &Bound<'_, PyType>) -> Self {
        PyCurrency { inner: CoreCurrency::GBP }
    }

    #[classmethod]
    fn jpy(_cls: &Bound<'_, PyType>) -> Self {
        PyCurrency { inner: CoreCurrency::JPY }
    }

    #[classmethod]
    fn chf(_cls: &Bound<'_, PyType>) -> Self {
        PyCurrency { inner: CoreCurrency::CHF }
    }

    #[classmethod]
    fn aud(_cls: &Bound<'_, PyType>) -> Self {
        PyCurrency { inner: CoreCurrency::AUD }
    }

    #[classmethod]
    fn cad(_cls: &Bound<'_, PyType>) -> Self {
        PyCurrency { inner: CoreCurrency::CAD }
    }
...
pub fn register_module(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add("USD", PyCurrency { inner: CoreCurrency::USD })?;
    m.add("EUR", PyCurrency { inner: CoreCurrency::EUR })?;
    m.add("GBP", PyCurrency { inner: CoreCurrency::GBP })?;
    m.add("JPY", PyCurrency { inner: CoreCurrency::JPY })?;
    m.add("CHF", PyCurrency { inner: CoreCurrency::CHF })?;
    m.add("AUD", PyCurrency { inner: CoreCurrency::AUD })?;
    m.add("CAD", PyCurrency { inner: CoreCurrency::CAD })?;

    m.add("__doc__", "Core financial primitives including Currency enum")?;
    Ok(())
}
